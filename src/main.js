...
function sanitizeCats(cats) {
  ...
  const base = { clientes: [], almacenes: [], tramitadores: seedTramitadores };
  const out = { ...base, ...(cats || {}) };
...
let state = {
  ...
  expandedClientesRef: {},
  expandedHistorial: {},
  asigDraft: { tipo: "previo", ref: "", guia: "", cliente: "", almacen: "", sector: "", tramitador: "" },
              ...
              .map(
                (r, ri) =>
                  `<tr>${PREVIO_FIELDS.map((f) =>
                    f.k === "observaciones" && state.userRole === "ejecutivo"
                      ? `<td><input value="${esc(r[f.k])}" placeholder="Agregar observación…" style="width:100%;min-width:140px;padding:5px 6px;border:1px solid var(--line);border-radius:6px;font-size:12.5px;" onblur="App.updateObservacion('previos','${d.fecha}',${ci},${ri},this.value)"/></td>`
                      : `<td>${esc(r[f.k])}</td>`
                  ).join("")}</tr>`
              )
              .join("") ||
              ...
              .map(
                (r, ri) =>
                  `<tr>${DESPACHO_FIELDS.map((f) =>
                    f.k === "observaciones" && state.userRole === "ejecutivo"
                      ? `<td><input value="${esc(r[f.k])}" placeholder="Agregar observación…" style="width:100%;min-width:140px;padding:5px 6px;border:1px solid var(--line);border-radius:6px;font-size:12.5px;" onblur="App.updateObservacion('despachos','${d.fecha}',${ci},${ri},this.value)"/></td>`
                      : `<td>${esc(r[f.k])}</td>`
                  ).join("")}</tr>`
              )
              .join("") ||
      ...
      <div class="captura-item" style="background:${atrasada ? "#FCDDD6" : "#FEF6F5"};border-color:${atrasada ? "#E8A79A" : "#F3C9C2"};">
        <div class="captura-head">
          <div class="meta-line">
            ...
            }${a.sector ? ` &nbsp;·&nbsp; <b>Sector:</b> ${esc(a.sector)}` : ""}<br/>
...
const App = {
  ...
  goAsignaciones() {
    ...
    state.view = "asignaciones";
    state.errorMsg = "";
    state.asigDraft = { tipo: "previo", ref: "", guia: "", cliente: "", almacen: "", sector: "", tramitador: (editableCats.tramitadores || [])[0] || "" };
    render();
  },
  ...
  async crearAsignacion() {
    ...
    state.asigDraft = { tipo: "previo", ref: "", guia: "", cliente: "", almacen: "", sector: "", tramitador };
    render();
    try {
