  ...
  <div class="login-wrap">
    <div class="login-card">
      ...
      <div class="user-pick">
        ${TRAMITADORES.map(
          (n) =>
            `<button class="user-pick-btn" onclick="App.selectTramitador('${esc(n)}')"><span class="user-avatar">${esc(
...
function viewAdminPassword() {
  ...
  <div class="login-wrap">
    <div class="login-card">
      <div class="login-mark"><img src="/logo-ow.jpg" alt="OW" /></div>
      <div class="login-title display">Acceso de Administrador</div>
...
function viewSelectAduana() {
  ...
  <div class="login-wrap">
    <div class="login-card">
      ...
      <div class="user-pick">
        ${ADUANAS.map(
          (a) => `<button class="user-pick-btn" onclick="App.selectAduana('${esc(a)}')"><span class="user-avatar">${esc(a.slice(0, 2))}</span> ${esc(a)}</button>`
        ).join("")}
      </div>
      <button class="btn btn-ghost btn-sm" style="margin-top:16px;" onclick="App.backToRoleSelect()">← Volver</button>
    </div>
...
function viewReview() {
  ...
  return `
    ...
    <div class="panel" style="margin-bottom:16px;">
      ...
      <div class="section-sub" style="margin-bottom:10px;">Operaciones que la persona de trámite deja pendientes ese día — la fecha del reporte indica cuándo se dejó pendiente.</div>
      <div class="table-wrap">
        <table class="rows-table">
...
const App = {
  ...
  submitAdminPassword() {
    ...
    if (val === ADMIN_PASSWORD) {
      state.errorMsg = "";
      App.pendingUser = "Administrador";
      App.pendingRole = "admin";
      App.goSelectAduana();
    } else {
  ...
  selectTramitador(nombre) {
    App.pendingUser = nombre;
    App.pendingRole = "tramite";
    App.goSelectAduana();
  },
  goSelectAduana() {
  ...
  selectAduana(aduana) {
    ...
    App.enterApp(App.pendingUser, App.pendingRole);
  },
  cambiarAduana() {
    state.view = "selectAduana";
    render();
