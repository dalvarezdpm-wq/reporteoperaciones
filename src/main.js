...
function statsProductividadPonderada(desde, hasta) {
  ...
  allCapturas()
    ...
    .forEach((c) => {
      ...
      if (!map[key]) map[key] = { nombre: nombreCanonico(t), puntos: 0, operaciones: 0 };
      (c.previos || []).forEach((r) => {
        map[key].puntos += puntosDificultad(r.tipoPrevio);
        map[key].operaciones += 1;
      });
      (c.despachos || []).forEach((r) => {
        map[key].puntos += puntosDificultad(r.dificultad);
        map[key].operaciones += 1;
      });
    });
