    ...
    <div class="panel" style="margin-bottom:20px;">
      ...
      <div style="margin-top:14px;">
        <div class="section-sub" style="margin-bottom:10px;">
          Pega aquí varias guías de un jalón — una por línea. Puedes pegar directo desde Excel (Guía, luego Cliente, luego Referencia en columnas separadas por tabulador),
          o escribir "guía, cliente, referencia" separado por comas. Cliente y Referencia son opcionales. El mismo Tipo, Sector y Tramitador que elijas abajo se aplica a todas.
        </div>
        ...
        <textarea id="asig_masiva_texto" rows="8" placeholder="875782762327	IMPRO	GLSI262480&#10;861234567890	BRP MEXICANA&#10;745609590220" style="width:100%;font-family:'IBM Plex Mono',monospace;font-size:13px;padding:10px;border:1px solid var(--line);border-radius:var(--radius);resize:vertical;"></textarea>
...
const App = {
  ...
  async crearAsignacionesMasivas() {
    const textarea = document.getElementById("asig_masiva_texto");
    if (!textarea) return;
    const lineas = textarea.value
      ...
      .filter(Boolean);
    if (lineas.length === 0) {
      state.asigMasivaMsg = "Pega al menos una guía para crear asignaciones.";
      render();
      return;
    }
    ...
    for (let i = 0; i < lineas.length; i++) {
      const partes = lineas[i].includes("\t") ? lineas[i].split("\t") : lineas[i].split(",");
      const guia = (partes[0] || "").trim();
      const cliente = (partes[1] || "").trim();
      const ref = (partes[2] || "").trim();
      if (!guia) {
        omitidas++;
        continue;
      }
    ...
    textarea.value = "";
    state.asigMasivaMsg = `✅ Se crearon ${creadas} asignación${creadas === 1 ? "" : "es"}` + (omitidas > 0 ? ` (${omitidas} línea(s) sin guía se omitieron).` : ".");
    render();
  },
