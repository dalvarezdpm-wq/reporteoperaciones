...
function topbar() {
  ...
  const sinTramitadorCount = Object.values(state.asignaciones || {}).filter((a) => a.estatus === "pendiente" && !(a.tramitador || "").trim()).length;
...
function viewPendientesGlobal() {
  ...
  return `
    ...
    <div class="panel" style="margin-bottom:20px;border-color:#EAD199;background:#FFFBF2;">
      ...
      <div class="section-sub" style="margin-bottom:12px;">Se crearon sin saber todavía quién las va a trabajar — elige a alguien para cada una, y aparecerá sola en el reporte de esa persona en cuanto entre a capturar.</div>
      ${sinTramitador
        .map((a) => {
          const dias = a.fechaCreacion ? Math.floor((Date.now() - new Date(a.fechaCreacion).getTime()) / 86400000) : 0;
          const esDeHoy = (a.fechaCreacion || "").slice(0, 10) === hoy;
          return `
      <div class="captura-item" style="background:${esDeHoy ? "#FEF6F5" : "#FBF3E3"};border-color:${esDeHoy ? "#F3C9C2" : "#EAD199"};margin-bottom:10px;">
        <div class="meta-line">
          ${
            esDeHoy
              ? `<span class="pill" style="background:#F8D9D4;color:var(--rojo);">⏳ Creada hoy</span>`
