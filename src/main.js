  ...
  for (const fecha of fechas) {
    ...
    try {
      if (entry.data === null) {
        ...
        // Mismo criterio seguro que saveReportDay: cada hoja se guarda por separado,
        // nunca se reescribe el día completo de un jalón — importante también aquí,
        // en el reintento automático al recuperar señal.
        const capturas = Array.isArray(entry.data.capturas) ? entry.data.capturas : [];
        for (const captura of capturas) {
          await fbSaveCapturaUnica(state.aduanaActiva, fecha, captura);
        }
      }
      clearPending(fecha);
      anySucceeded = true;
      ...
      (c.pendientes || []).forEach((p) => {
        ...
        const yaLlego = todosLosDias.some(
          (otroDia) =>
            ...
            (otroDia.capturas || []).some((c2) => {
              if (!mismoNombre(c2.tramitador, tramitador)) return false;
              const enCualquierGrupo = (grupo) => (c2[grupo] || []).some((p2) => p2.origenFecha === origen && (p2.guia || "").trim() === guia);
              return enCualquierGrupo("pendientes") || enCualquierGrupo("enProceso") || enCualquierGrupo("previos") || enCualquierGrupo("despachos") || enCualquierGrupo("revalidadas");
            })
        );
        if (!yaLlego) {
