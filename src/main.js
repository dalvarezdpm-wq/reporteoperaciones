...
function render() {
  try {
    ...
    else if (state.view === "selectCoordinador") root.innerHTML = viewSelectCoordinador();
...
function viewSelectCoordinador() {
  return `
  <div class="login-wrap">
    <div class="login-card">
      <div class="login-mark"><img src="logo-ow.jpg" alt="OW" /></div>
      <div class="login-title display">¿Quién eres?</div>
      <div class="login-sub">Elige tu nombre — así quedará marcado en cada cosa que hagas como Coordinación</div>
      <div class="user-pick">
        ${(editableCats.coordinadores || []).map(
          (n) =>
            `<button class="user-pick-btn" onclick="App.submitCoordinador('${esc(n)}')"><span class="user-avatar">${esc(
              n.split(" ").map((p) => p[0]).join("").slice(0, 2).toUpperCase()
            )}</span> ${esc(n)}</button>`
        ).join("")}
      </div>
              ...
              .map(
                (r, ri) =>
                  `<tr>${REVALIDADA_FIELDS.map((f) => `<td>${esc(r[f.k])}</td>`).join("")}<td>${
                    r.foto
                      ? `<img src="${r.foto}" onclick="App.verFotoRevalidadaDetalle('${d.fecha}',${ci},${ri})" title="Ver foto" style="width:30px;height:30px;object-fit:cover;border-radius:5px;cursor:pointer;border:1px solid var(--line);"/>`
                      : "—"
                  }</td></tr>`
              )
              .join("")}</tbody>
      ...
      <div class="panel" style="margin-bottom:14px;">
        ...
        ${
          k === "tramitadores"
            ...
            : k === "coordinadores"
            ? `<div style="color:var(--muted);font-size:11.5px;margin-bottom:8px;">Usan la misma contraseña de Coordinación de siempre — si hay nombres aquí, después de la contraseña se les pide elegir cuál de ellos es, para quedar identificados con su propio nombre. Si esta lista queda vacía, se entra directo como "Administrador", sin ese paso extra.</div>`
            : ""
        }
        ${
          necesitaBuscador
...
function todasLasSolicitudesEliminacion() {
  ...
  Object.values(state.reports || {}).forEach((day) => {
    (day.capturas || []).forEach((c, ci) => {
      ["previos", "despachos", "revalidadas", "pendientes"].forEach((grupo) => {
        (c[grupo] || []).forEach((row, idx) => {
          if (row._solicitudEliminacion) {
            list.push({ fecha: day.fecha, ci, grupo, idx, row, tipo: "renglon", tramitador: c.tramitador });
          }
          if (row._solicitudQuitarFoto) {
            list.push({ fecha: day.fecha, ci, grupo, idx, row, tipo: "foto", tramitador: c.tramitador });
          }
        });
      });
    });
    ...
    ${
      ...
      <div class="panel" style="margin-bottom:12px;">
        ...
        <div style="display:flex;gap:8px;margin-top:10px;">
          <button class="btn btn-primary btn-sm" onclick="App.autorizarEliminacionDesdeGlobal('${item.fecha}',${item.ci},'${item.grupo}',${item.idx},'${item.tipo}')">✅ Autorizar</button>
          <button class="btn btn-ghost btn-sm" onclick="App.rechazarEliminacionDesdeGlobal('${item.fecha}',${item.ci},'${item.grupo}',${item.idx},'${item.tipo}')">❌ Rechazar</button>
        </div>
      </div>`;
            })
            .join("")
    ...
    ${
      completadas.length === 0
        ...
        : (() => {
            ...
            return completadas
              .map((a) => {
                const sinRastro = asignacionSinRastro(a, indice);
                return `
...
const App = {
  ...
  submitAdminPassword() {
    ...
    if (val === (ADMIN_PASSWORDS[state.aduanaActiva] || ADMIN_PASSWORDS.GDL)) {
      state.errorMsg = "";
      // Si esta aduana tiene varios coordinadores dados de alta en el catálogo, se
      // pide elegir cuál de ellos es, para que cada quien quede identificado con su
      // propio nombre en vez de un genérico "Administrador" para todos.
      if ((editableCats.coordinadores || []).length > 0) {
        state.view = "selectCoordinador";
        render();
      } else {
  ...
  submitCoordinador(nombre) {
    if (!nombre) return;
    state.errorMsg = "";
    App.enterApp(nombre, "admin");
  },
  ...
  async autorizarEliminacionDesdeGlobal(fecha, ci, grupo, idx, tipo) {
    const day = state.reports[fecha];
    const c = day && day.capturas && day.capturas[ci];
    const row = c && c[grupo] && c[grupo][idx];
    if (!row) return;
    if (tipo === "foto") {
      delete row.foto;
   ...
   * queda tal cual, solo se le quita la marca de "pendiente de autorizar". */
  async rechazarEliminacionDesdeGlobal(fecha, ci, grupo, idx, tipo) {
    const day = state.reports[fecha];
    const c = day && day.capturas && day.capturas[ci];
    const row = c && c[grupo] && c[grupo][idx];
    if (!row) return;
  ...
  async rechazarEliminacionRenglon(group, i) {
    ...
    if (
      state.editingIndex !== null &&
      state.editingFecha &&
      state.reports[state.editingFecha] &&
      state.reports[state.editingFecha].capturas &&
      state.reports[state.editingFecha].capturas[state.editingIndex]
    ) {
      state.reports[state.editingFecha].capturas[state.editingIndex] = state.currentCaptura;
      persistLocalCache();
      try {
  ...
  verFotoRevalidadaDetalle(fecha, ci, ri) {
    const day = state.reports[fecha];
    const r = day && day.capturas && day.capturas[ci] && day.capturas[ci].revalidadas && day.capturas[ci].revalidadas[ri];
    if (!r || !r.foto) return;
    const w = window.open("", "_blank");
