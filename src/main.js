...
function puntosDificultad(valor) {
  return PUNTOS_DIFICULTAD[(valor || "").trim().toUpperCase()] || PUNTOS_DIFICULTAD.A;
}
 ...
 * a propósito (no tienen campo de dificultad). */
function statsProductividadPonderada(desde, hasta) {
  ...
  allCapturas()
    ...
    .forEach((c) => {
      ...
      if (!t) return;
      if (!map[t]) map[t] = { puntos: 0, operaciones: 0 };
      (c.previos || []).forEach((r) => {
        map[t].puntos += puntosDificultad(r.tipoPrevio);
        map[t].operaciones += 1;
      });
      (c.despachos || []).forEach((r) => {
        map[t].puntos += puntosDificultad(r.dificultad);
        map[t].operaciones += 1;
      });
    });
  ...
  return `
    ...
    <div class="dashboard-grid" style="margin-bottom:20px;">
      ...
      <div class="panel">
        ...
        <div class="section-sub" style="margin-bottom:10px;">Previos + despachos, valorados de A (25 pts) a D (100 pts) — las revalidadas no cuentan aquí</div>
        ${
    ...
    <div class="panel" style="margin-bottom:16px;overflow-x:auto;">
      ...
      ${
        ponderada.length === 0
          ...
          : `
      <table class="mini-table" style="min-width:480px;">
        <thead><tr><th>Tramitador</th><th>Puntos totales</th><th>Operaciones</th><th>Promedio por operación</th></tr></thead>
        <tbody>
        ${ponderada
          .map(
            (s) => `<tr>
...
const App = {
  ...
  editarAsignacion(id) {
    ...
    state.asigDraft = {
      ...
      tramitador: a.tramitador || (editableCats.tramitadores || [])[0] || "",
    };
    state.errorMsg = "";
    render();
