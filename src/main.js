...
async function migrarPendientesAlDiaDeHoy(tramitador) {
  ...
  //
  // Se arma primero UN solo índice (guía+origen -> fechas donde existe una copia) en
  // vez de volver a recorrer todos los días por cada pendiente bloqueado — con meses de
  // historial acumulado, repetir ese recorrido por cada uno se vuelve cada vez más
  // lento (podía llegar a sentirse como que la app "no reacciona" al crear un reporte
  // nuevo), mientras que armar el índice una sola vez es rápido sin importar cuánto
  // historial haya. ---
  const indiceCopias = {};
    ...
    (day.capturas || []).forEach((c) => {
      ...
      ["pendientes", "enProceso", "previos", "despachos", "revalidadas"].forEach((grupo) => {
        (c[grupo] || []).forEach((p) => {
          ...
          if (!guia || !p.origenFecha) return;
          const key = p.origenFecha + "||" + guia;
          if (!indiceCopias[key]) indiceCopias[key] = [];
          indiceCopias[key].push(day.fecha);
        });
      });
    });
  });
  todosLosDias.forEach((day) => {
    ...
    (day.capturas || []).forEach((c) => {
      ...
      (c.pendientes || []).forEach((p) => {
        ...
        const fechasConCopia = indiceCopias[origen + "||" + guia] || [];
...
const App = {
  ...
  async eliminarPendienteGlobal(fecha, capturaId, guia) {
    ...
    const origenFechaRow = pend.origenFecha;
    ...
    if (origenFechaRow) {
      ...
      if (origenDay) {
        ...
        (origenDay.capturas || []).forEach((c) => {
          ...
          (c.pendientes || []).forEach((p) => {
            if (p.bloqueado && p.origenFecha === origenFechaRow && normalizarNombre(p.guia) === normalizarNombre(guia)) {
              p.descartado = true;
              cambiado = true;
            }
          });
        });
        if (cambiado && origenFechaRow !== fecha) {
          try {
