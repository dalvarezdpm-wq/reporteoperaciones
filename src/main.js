 ...
 * despacho o pendiente) de CUALQUIER reporte de la aduana activa — para autocompletarla
 * cuando se vuelve a capturar la misma guía (ej. un 2do previo de la misma guía) y así
 * no haya que volver a teclearla ni buscarla a mano. Devuelve la más reciente. */
function buscarReferenciaPorGuia(guia, idCapturaActual) {
  const g = (guia || "").trim();
  if (!g) return "";
  let mejor = null;
  allCapturas().forEach((c) => {
    if (c.id === idCapturaActual) return; // no comparar un renglón contra sí mismo
    [...(c.previos || []), ...(c.despachos || []), ...(c.pendientes || [])].forEach((r) => {
      if ((r.guia || "").trim() !== g) return;
...
const App = {
  ...
  autocompletarRefPorGuia(group, i) {
    try {
      ...
      const encontrada = buscarReferenciaPorGuia(guia, state.currentCaptura.id);
      if (encontrada) {
        row.ref = encontrada;
        render();
      }
  ...
  days.forEach((d) =>
    (d.capturas || []).forEach((c) =>
      resumenRows.push({
        ...
        "Total revalidadas": (c.revalidadas || []).length,
  ...
  days.forEach((d) =>
    (d.capturas || []).forEach((c) =>
      (c.revalidadas || []).forEach((r) => {
        const row = { Fecha: c.fecha, Tramitador: c.tramitador };
        REVALIDADA_FIELDS.forEach((f) => (row[f.label] = r[f.k] || ""));
        revalidadasRows.push(row);
      })
    )
  );
  const wsRevalidadas = XLSX.utils.json_to_sheet(revalidadasRows);
  XLSX.utils.book_append_sheet(wb, wsRevalidadas, "Revalidadas");
