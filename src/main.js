...
/** Algunas aduanas dejan que cada tramitador se autoasigne las guías que va a trabajar
 * de la "bolsa" de asignaciones sin tramitador (ej. Toluca: Coordinación asigna masivo
 * sin decidir quién hace cada una, y cada tramitador entra y agarra las suyas). Si la
 * aduana activa aparece aquí, se habilita esa pantalla para el rol Trámite. */
const AUTOASIGNACION_ADUANAS = ["TOLUCA"];
...
function comprimirImagenArchivo(file, maxAncho = 1000, calidad = 0.6) {
 ...
 * operativo" pertenece ese momento, con el mismo corte de las 8:00 PM. Sirve para saber
 * si algo se creó "hoy" (en el sentido operativo) o si ya cruzó el cierre del día sin
 * resolverse — ej. una asignación sin tramitador creada a las 9pm ya cuenta como de
 * "mañana", igual que pasaría con un pendiente sin resolver a esa hora. */
...
const App = {
  ...
  async crearAsignacionesMasivas() {
    ...
    if (pedimentos.length && pedimentos.length !== guias.length) avisos.push(`Pedimento tenía ${pedimentos.length} renglón(es) en vez de ${guias.length}`);
