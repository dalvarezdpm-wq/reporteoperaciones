...
import { PREVIO_FIELDS, DESPACHO_FIELDS, REVALIDADA_FIELDS, PENDIENTE_FIELDS, EN_PROCESO_FIELDS, emptyCaptura } from "./fields.js";
import {
  loadReports as fbLoadReports,
  saveReportDay as fbSaveReportDay,
  deleteReportDay as fbDeleteReportDay,
  saveCapturaUnica as fbSaveCapturaUnica,
  loadCapturaUnica as fbLoadCapturaUnica,
  deleteCapturaUnica as fbDeleteCapturaUnica,
...
function normalizeCaptura(c) {
  ...
  if (!Array.isArray(c.enProceso)) c.enProceso = []; // reportes de antes de este campo simplemente no tenían nada "en proceso"
  ...
  if (fijo) {
    ["previos", "despachos", "revalidadas", "pendientes", "enProceso"].forEach((grupo) => {
      (c[grupo] || []).forEach((r) => {
...
let _persistLocalCacheTimer = null;
/** Guarda una copia de reportes/catálogos en este dispositivo, para el modo sin
 * conexión. Ahora que los cambios llegan en tiempo real (pueden ser muchos por minuto
 * si varias personas trabajan a la vez), esto se limita a guardar como máximo una vez
 * cada 3 segundos — de lo contrario, cada cambio ajeno dispararía una escritura pesada
 * al almacenamiento del celular, sin necesidad real (el respaldo offline no necesita
 * ser instantáneo, solo razonablemente reciente). */
function persistLocalCache() {
  if (_persistLocalCacheTimer) return; // ya hay una escritura programada muy pronto, no hace falta otra
  _persistLocalCacheTimer = setTimeout(() => {
    _persistLocalCacheTimer = null;
    try {
...
async function loadReports() {
  ...
  state.loading = false;
  render();
  if (navigator.onLine) iniciarEscuchaEnTiempoReal();
 ...
 */
// Funciones para dejar de escuchar los cambios en tiempo real de la aduana actual —
// se llaman al cambiar de aduana o cerrar sesión, para no quedarse escuchando cambios
// de una aduana que ya no es la activa.
let detenerEscuchaReportes = null;
let detenerEscuchaAsignaciones = null;

/** Fechas que en este momento se están guardando (ver saveReportDay) — mientras un día
 * tenga un contador mayor a 0 aquí, la escucha en tiempo real NO reemplaza esa fecha
 * específica con lo que llegue del servidor, aunque sí actualiza todas las demás con
 * normalidad. Es un CONTADOR (no solo sí/no) porque el mismo día se puede estar
 * guardando más de una vez encimado (ej. reasignar toca el día viejo y el día nuevo, y
 * alguna función interna puede volver a llamar a un guardado del mismo día) — así, el
 * primero en terminar no le quita la protección al segundo que todavía sigue en curso.
 *
 * Esto evita una condición de carrera real: si alguien mutaba state.reports[fecha] y
 * luego (antes de que terminara de guardar) llegaba una actualización en tiempo real
 * que reemplazaba TODO state.reports de golpe, el guardado terminaba actuando sobre
 * datos frescos que nunca tuvieron el cambio que se quería guardar — perdiéndolo por
 * completo, sin ningún aviso. Pasaba más seguido justo después de una reconexión (por
 * ejemplo, al recuperar el acceso tras una interrupción de Firebase), cuando llega de
 * golpe una ráfaga de actualizaciones pendientes. */
const diasGuardandoAhora = new Map();
function marcarGuardando(fecha) {
  diasGuardandoAhora.set(fecha, (diasGuardandoAhora.get(fecha) || 0) + 1);
}
function desmarcarGuardando(fecha) {
  const n = (diasGuardandoAhora.get(fecha) || 0) - 1;
  if (n <= 0) diasGuardandoAhora.delete(fecha);
  else diasGuardandoAhora.set(fecha, n);
}

/** Empieza a escuchar reportes y asignaciones de la aduana activa EN TIEMPO REAL — a
 * diferencia de preguntar cada cierto tiempo (lo que se hacía antes, cada 25 segundos),
 * esto avisa de inmediato en cuanto alguien más guarda o cambia algo en el servidor, sin
 * ningún retraso de por medio. Se evita interrumpir a alguien que esté escribiendo en
 * ese momento (pantalla "review"): los datos igual se actualizan por dentro para no
 * quedarse atrás, pero la pantalla no se vuelve a dibujar hasta que salga de ahí. */
function iniciarEscuchaEnTiempoReal() {
  detenerEscuchaEnTiempoReal(); // por si ya había una activa de otra aduana
  if (!state.aduanaActiva) return;
  const manejarErrorEscucha = (fuente) => (error) => {
    // Si la escucha misma falla (poco común — problema de conexión persistente, por
    // ejemplo), no se debe quedar la app sin enterarse de cambios en silencio. Se avisa
    // una sola vez, y se cae de vuelta a preguntar cada cierto tiempo como respaldo,
    // en vez de depender solo de la escucha en tiempo real que ya falló.
    if (!state._avisoEscuchaFallidaMostrado) {
      state._avisoEscuchaFallidaMostrado = true;
      state.errorMsg = "Se perdió la sincronización en tiempo real (" + (error && error.message ? error.message : "error de conexión") + ") — la app va a seguir revisando cambios cada cierto tiempo mientras tanto.";
      render();
    }
    if (!state._respaldoSondeoActivo) {
      state._respaldoSondeoActivo = setInterval(() => {
        if (navigator.onLine && state.aduanaActiva) backgroundRefreshRespaldo();
      }, 25000);
    }
  };
  detenerEscuchaReportes = subscribeToReports(
    state.aduanaActiva,
    (reportes) => {
      if (diasGuardandoAhora.size > 0) {
        // Se conserva tal cual cualquier día que se esté guardando ahora mismo — se
        // actualizan todos los demás días con lo nuevo, pero esos puntuales no, para no
        // arriesgarse a perder la mutación que todavía no ha terminado de guardarse.
        Object.keys(reportes).forEach((fecha) => {
          if (diasGuardandoAhora.has(fecha) && state.reports[fecha]) {
            reportes[fecha] = state.reports[fecha];
          }
        });
        diasGuardandoAhora.forEach((_cuenta, fecha) => {
          if (!reportes[fecha] && state.reports[fecha]) reportes[fecha] = state.reports[fecha];
        });
      }
      state.reports = reportes;
      applyPendingOverlay();
      persistLocalCache();
      if (state.view !== "review") render();
    },
    manejarErrorEscucha("reportes")
  );
  detenerEscuchaAsignaciones = subscribeToAsignaciones(
    state.aduanaActiva,
    (asignaciones) => {
      notificarCambiosAsignaciones(asignaciones);
      state.asignaciones = asignaciones;
      if (state.view !== "review") render();
    },
    manejarErrorEscucha("asignaciones")
  );
}
/** Respaldo por si la escucha en tiempo real llegó a fallar (ver manejarErrorEscucha) —
 * mismo criterio de siempre: no refrescar mientras alguien esté en plena captura. */
async function backgroundRefreshRespaldo() {
  if (state.view === "review") return;
  try {
    const reportes = await fbLoadReports(state.aduanaActiva);
    // Misma protección que la escucha en tiempo real: no reemplazar un día que se esté
    // guardando ahora mismo, para no perder una mutación a medio camino.
    if (diasGuardandoAhora.size > 0) {
      diasGuardandoAhora.forEach((_cuenta, fecha) => {
        if (state.reports[fecha]) reportes[fecha] = state.reports[fecha];
      });
    }
    state.reports = reportes;
    applyPendingOverlay();
  ...
  try {
    const nuevas = await fbLoadAsignaciones(state.aduanaActiva);
    notificarCambiosAsignaciones(nuevas);
    state.asignaciones = nuevas;
  } catch (e) {}
  render();
}
function detenerEscuchaEnTiempoReal() {
  if (state._respaldoSondeoActivo) {
    clearInterval(state._respaldoSondeoActivo);
    state._respaldoSondeoActivo = null;
  }
  if (detenerEscuchaReportes) {
    try {
      detenerEscuchaReportes();
    } catch (e) {}
    detenerEscuchaReportes = null;
  }
  if (detenerEscuchaAsignaciones) {
    try {
      detenerEscuchaAsignaciones();
    } catch (e) {}
    detenerEscuchaAsignaciones = null;
  }
}
...
async function backgroundRefresh() {
  ...
  // Reportes y asignaciones ya no se preguntan aquí — se escuchan en tiempo real (ver
  // iniciarEscuchaEnTiempoReal). Esto solo reintenta subir lo que se haya quedado
  // pendiente por falta de señal, como respaldo extra al intervalo dedicado a eso.
  if (pendingFechas().length > 0) {
    try {
      await processPendingQueue();
    } catch (e) {}
  ...
  for (const fecha of fechas) {
    ...
    try {
      if (entry.data === null) {
        ...
        // Mismo criterio seguro que saveReportDay: cada hoja se guarda por separado, y
        // se fusiona con lo que ya haya en el servidor antes de sobrescribir — la copia
        // pendiente por falta de señal puede ser más vieja que otra cosa que ya se haya
        // guardado ahí mientras tanto (desde otro dispositivo de la misma persona, por
        // ejemplo), y no queremos perder eso al subir por fin lo que quedó pendiente.
        const capturas = Array.isArray(entry.data.capturas) ? entry.data.capturas : [];
        for (const captura of capturas) {
          let paraGuardar = captura;
          try {
            const fresca = await fbLoadCapturaUnica(state.aduanaActiva, fecha, captura.id);
            paraGuardar = fusionarConServidor(fresca, captura, captura._recienBorrados);
          } catch (e) {}
          await fbSaveCapturaUnica(state.aduanaActiva, fecha, paraGuardar);
        }
      }
      clearPending(fecha);
      anySucceeded = true;
...
/** Guarda todas las hojas que esta sesión tiene en memoria para ese día — pero cada
 * una POR SEPARADO, en su propio espacio (ver saveCapturaUnica en storage.js), nunca
 * reescribiendo el día completo de un jalón. Esto es el corazón de la corrección para
 * cuando varias personas trabajan en la app al mismo tiempo: como cada hoja se guarda
 * en su propio nodo, guardar la hoja de una persona nunca puede borrar ni pisar la hoja
 * de otra persona que esta sesión ni siquiera conozca (por ejemplo, si se guardó
 * mientras esta sesión llevaba un rato editando sin recargar). */
/** Combina la copia local de una hoja con lo que YA existe de verdad en el servidor en
 * ese momento — en vez de sobrescribir a ciegas. Para Previos/Despachos/Revalidadas/
 * Pendientes/En proceso: cualquier renglón que exista en el servidor (por su Guía) pero
 * no esté en la copia local se conserva también, en vez de perderse. Esto es para el
 * caso de que la MISMA persona tenga la app abierta en dos lugares a la vez (celular y
 * computadora, dos pestañas, o se le recargó a medias por mala señal) — cada sesión
 * guardaba "su propia versión completa" sin saber lo que la otra ya había guardado, y
 * la que guardaba al final borraba sin querer lo que la otra había agregado. La copia
 * local manda para todo lo demás (su propio orden, sus propias ediciones a lo que ya
 * tenían ambas). */
/** Combina la copia local de una hoja con lo que YA existe de verdad en el servidor en
 * ese momento — en vez de sobrescribir a ciegas. Para Previos/Despachos/Revalidadas/
 * Pendientes/En proceso: cualquier renglón que exista en el servidor (por su Guía) pero
 ...
 * tenían ambas).
 *
 * "exclusiones" (opcional, arreglo de {grupo, guia}) es la salvedad importante: un
 * renglón que se ACABA de borrar a propósito se ve EXACTAMENTE IGUAL, para esta lógica,
 * que uno que el servidor tiene y la copia local todavía no conoce — sin esta lista, la
 * misma protección que evita perder cosas terminaría resucitando cualquier borrado, ya
 * que "el servidor lo tiene y la copia local ya no" es la señal que usa para recuperar
 * algo. Cualquier guía en esta lista nunca se recupera del servidor, sin importar que
 * siga apareciendo ahí. */
function fusionarConServidor(fresca, local, exclusiones) {
  if (!fresca) {
    const solaLocal = { ...local };
    delete solaLocal._recienBorrados;
    return solaLocal; // nada en el servidor todavía — la copia local manda completa
  }
  const resultado = { ...local };
  delete resultado._recienBorrados;
  const excluirPorGrupo = {};
  (exclusiones || []).forEach((x) => {
    if (!excluirPorGrupo[x.grupo]) excluirPorGrupo[x.grupo] = new Set();
    excluirPorGrupo[x.grupo].add(x.guia);
  });
  ["previos", "despachos", "revalidadas", "pendientes", "enProceso"].forEach((grupo) => {
    const localRows = Array.isArray(local[grupo]) ? local[grupo] : [];
    const freshRows = Array.isArray(fresca[grupo]) ? fresca[grupo] : [];
    const guiasLocales = new Set(localRows.map((r) => (r.guia || "").trim()).filter(Boolean));
    const excluidas = excluirPorGrupo[grupo] || new Set();
    const extras = freshRows.filter((r) => {
      const g = (r.guia || "").trim();
      return g && !guiasLocales.has(g) && !excluidas.has(g);
    });
    resultado[grupo] = extras.length ? [...localRows, ...extras] : localRows;
  });
  return resultado;
}
...
async function saveReportDay(fecha) {
  ...
  const capturas = day && Array.isArray(day.capturas) ? day.capturas : [];
  if (!navigator.onLine) {
    markPending(fecha, day);
    return;
  }
  marcarGuardando(fecha);
  try {
    for (const captura of capturas) {
      let paraGuardar = captura;
      const exclusiones = captura._recienBorrados;
      try {
        const fresca = await fbLoadCapturaUnica(state.aduanaActiva, fecha, captura.id);
        paraGuardar = fusionarConServidor(fresca, captura, exclusiones);
      } catch (e) {
        // Si no se pudo revisar el servidor primero (ej. falla puntual de red), se
        // guarda la copia local tal cual — mejor eso a no guardar nada.
      }
      await fbSaveCapturaUnica(state.aduanaActiva, fecha, paraGuardar);
      // Se refleja la fusión de vuelta en el objeto local, para que la pantalla no se
      // quede mostrando menos renglones de los que en realidad ya quedaron guardados.
      Object.assign(captura, paraGuardar);
      delete captura._recienBorrados; // ya se usó para esta vuelta, no hace falta conservarla
    }
    clearPending(fecha);
  } catch (e) {
    markPending(fecha, day);
...
async function migrarPendientesAlDiaDeHoy(tramitador) {
  ...
  // una copia real en ningún día posterior — buscando en TODOS los lugares donde ese
  // trabajo pudo haber quedado resuelto: la copia viva en Pendientes, en "En proceso"
  // (movimiento en curso), o ya subido de verdad a Previos/Despachos/Guías revalidadas.
  // Si solo se buscara en Pendientes (como antes), cualquier cosa que ya se hubiera
  // finalizado (y por lo tanto ya no está en Pendientes) se malinterpretaría como "nunca
  // llegó", y el pendiente viejo se destrababa y se volvía a traer de regreso, aunque el
  // trabajo ya estuviera bien terminado.
  //
  // Se arma primero UN solo índice (guía -> fechas donde existe una copia, en cualquier
  // grupo relevante) en vez de volver a recorrer todos los días por cada pendiente
  // bloqueado — con meses de historial acumulado, repetir ese recorrido por cada uno se
  // vuelve cada vez más lento, mientras que armar el índice una sola vez es rápido sin
  // importar cuánto historial haya.
  //
  // Se compara SOLO por guía (no también por fecha de origen exacta) a propósito: si el
  // trabajo se resolvió por un camino distinto al formal (ej. se capturó directo en
  // Despachos en vez de pasar por "En proceso"), esa guía nunca habría quedado marcada
  // con la misma fecha de origen que el pendiente viejo, y una comparación más estricta
  // fallaría en detectar que sí se resolvió — trayendo de regreso algo que en realidad
  // ya estaba bien terminado. ---
  const indiceCopias = {};
  todosLosDias.forEach((day) => {
    (day.capturas || []).forEach((c) => {
      if (!mismoNombre(c.tramitador, tramitador)) return;
      ["pendientes", "enProceso", "previos", "despachos", "revalidadas"].forEach((grupo) => {
        (c[grupo] || []).forEach((p) => {
          const guia = (p.guia || "").trim();
          if (!guia) return;
          if (!indiceCopias[guia]) indiceCopias[guia] = [];
          indiceCopias[guia].push(day.fecha);
        });
      });
    });
  });
  todosLosDias.forEach((day) => {
    ...
    (day.capturas || []).forEach((c) => {
      ...
      (c.pendientes || []).forEach((p) => {
        ...
        const fechasConCopia = indiceCopias[guia] || [];
        const yaLlego = fechasConCopia.some((f) => f > day.fecha);
        if (!yaLlego) {
 ...
 * "Traer asignaciones pendientes" del formulario de captura. Revisa TODOS los lugares
 * donde esa guía ya pudo haber quedado cubierta dentro de esta misma hoja — Pendientes,
 * "En proceso" (ya se está trabajando, aunque no se haya finalizado), y los controles
 * finales — para no volver a ofrecerla y arriesgarse a traerla duplicada. */
function asignacionesPendientesParaFormulario(tramitador) {
  const t = (tramitador || "").trim();
  if (!t) return [];
  const c = state.currentCaptura;
  const yaCubierta = new Set();
  ["pendientes", "enProceso", "previos", "despachos", "revalidadas"].forEach((grupo) => {
    (c[grupo] || []).forEach((r) => {
      const g = (r.guia || "").trim();
      if (g) yaCubierta.add(g);
    });
  });
  return Object.entries(state.asignaciones || {})
    .map(([id, a]) => ({ id, ...a }))
    .filter((a) => a.estatus === "pendiente" && mismoNombre(a.tramitador, t) && !yaCubierta.has((a.guia || "").trim()));
}
 ...
 * Si está editando activamente su propia hoja de hoy, se revisa esa copia en memoria
 * (refleja cambios todavía sin guardar); si no, se revisa la copia ya guardada de hoy. */
function misAsignacionesPendientes() {
  if (state.userRole !== "tramite") return [];
  const t = (state.user || "").trim();
  const hoy = todayStr();
  let capturaHoy = null;
  if (state.currentCaptura && state.currentCaptura.fecha === hoy && mismoNombre(state.currentCaptura.tramitador, t)) {
    capturaHoy = state.currentCaptura;
  } else {
    const dia = state.reports[hoy];
    capturaHoy = dia && (dia.capturas || []).find((c) => mismoNombre(c.tramitador, t));
  }
  const yaCubierta = new Set();
  if (capturaHoy) {
    ["pendientes", "enProceso", "previos", "despachos", "revalidadas"].forEach((grupo) => {
      (capturaHoy[grupo] || []).forEach((r) => {
        const g = (r.guia || "").trim();
        if (g) yaCubierta.add(g);
      });
    });
...
function render() {
  try {
    ...
    else if (state.view === "loadingAduana")
      root.innerHTML = `<div class="login-wrap"><div class="login-card">${loadingBlock().replace("Cargando reportes…", "Cargando…")}</div></div>`;
    else {
      root.innerHTML =
        topbar() +
...
/** Arma una casilla de búsqueda + cuadrícula de nombres en orden alfabético — para que
 * las pantallas donde Trámite/Ejecutivo/Coordinador eligen su nombre se vean
 * organizadas incluso con muchas personas, en vez de un solo listón vertical
 * interminable. El filtro de búsqueda actúa directo sobre la página con JavaScript
 * (sin pasar por el ciclo normal de la app), para no perder el foco del cursor a media
 * escritura — con muy pocos nombres (6 o menos) ni siquiera aparece, no hace falta. */
function selectorDeNombres(nombres, nombreFuncionOnClick) {
  const ordenados = [...nombres].sort((a, b) => a.localeCompare(b, "es"));
  const wrapId = "selector_" + Math.random().toString(36).slice(2, 8);
  return `
    <div id="${wrapId}">
      ${
        ordenados.length > 6
          ? `<input type="text" class="user-search-input" placeholder="🔍 Busca tu nombre…" oninput="
              var q = this.value.trim().toLowerCase();
              var contenedor = document.getElementById('${wrapId}');
              var visibles = 0;
              contenedor.querySelectorAll('.user-pick-btn').forEach(function(b){
                var coincide = b.dataset.nombre.indexOf(q) !== -1;
                b.style.display = coincide ? '' : 'none';
                if (coincide) visibles++;
              });
              contenedor.querySelector('.user-pick-empty').style.display = visibles === 0 ? '' : 'none';
            "/>`
          : ""
      }
      <div class="user-pick-grid">
        ${ordenados
          .map(
            (n) =>
              `<button class="user-pick-btn" data-nombre="${esc(n.toLowerCase())}" onclick="App.${nombreFuncionOnClick}('${esc(n)}')">
                <span class="user-avatar">${esc(
          ...
          .join("")}
        <div class="user-pick-empty" style="display:none;">Nadie coincide con esa búsqueda</div>
      </div>
    </div>
  `;
}
...
function viewSelectTramitador() {
  ...
  <div class="login-wrap">
    <div class="login-card">
      ...
      ${selectorDeNombres(editableCats.tramitadores || [], "chooseTramitador")}
...
function viewSelectCoordinadorLogin() {
  ...
  <div class="login-wrap">
    <div class="login-card">
      ...
      ${selectorDeNombres(editableCats.coordinadores || [], "chooseCoordinadorLogin")}
      <button class="btn btn-ghost btn-sm" style="margin-top:16px;" onclick="App.backToRoleSelect()">← Volver</button>
    </div>
  </div>`;
}
    ...
    <div class="login-card">
      ...
      ${
        (editableCats.ejecutivos || []).length === 0
          ...
          : selectorDeNombres(editableCats.ejecutivos || [], "submitEjecutivoName")
      }
...
function viewReview() {
  ...
  return `
    ...
    <div class="panel" style="margin-bottom:16px;">
      ...
      <div class="section-sub" style="margin-bottom:10px;">Operaciones que la persona de trámite deja pendientes ese día. Si tienen Tipo (previo/despacho/revalidada), aparece el botón "🔄 En proceso" para moverlas a la sección de abajo cuando se empiecen a trabajar.</div>
      <div class="table-wrap">
        <table class="rows-table">
      ...
      ${(() => {
        ...
        // botón para moverlos a "En proceso" a todos juntos de un jalón. Solo aplica a
        // Despacho (normalmente una sola persona con todo el consolidado) — Previo se deja
        // siempre individual, porque ahí sí pueden repartirse entre varios tramitadores.
        ...
        return `<div style="margin-bottom:10px;display:flex;flex-direction:column;gap:6px;">
          ${gruposConDosOMas
            .map(([loteId, idxs]) => {
              ...
              return `<button class="btn btn-primary btn-sm" style="align-self:flex-start;" onclick="App.marcarLoteEnProceso('${loteId}')">🔄 Mover TODO este lote a En proceso (${idxs.length} guías${
                cliente ? ` — ${esc(cliente)}` : ""
              })</button>`;
            })
            .join("")}
      ...
      <div class="subhead">5. Movimientos en proceso</div>
      <div class="section-sub" style="margin-bottom:10px;">Pendientes que ya se empezaron a trabajar, pero todavía no se dan por terminados. Al darle "✅ Finalizado", sube de verdad al control de Previos, Despachos o Guías revalidadas que corresponda.</div>
      <div class="table-wrap">
        <table class="rows-table">
          <thead><tr>${EN_PROCESO_FIELDS.map((f) => `<th>${f.label}</th>`).join("")}<th></th></tr></thead>
          <tbody>${(c.enProceso || []).map((row, i) => rowHtml(row, i, "enProceso", EN_PROCESO_FIELDS)).join("")}</tbody>
        </table>
      </div>
      ${
        (c.enProceso || []).length === 0
          ? `<div style="color:var(--muted);font-size:12px;">Todavía no hay nada en proceso — usa el botón "🔄 En proceso" de un pendiente de arriba para moverlo aquí.</div>`
          : ""
      }

      <div class="subhead">6. Otras actividades</div>
        ...
        ${
          row.foto
            ...
            : `<label title="Tomar o subir foto de la guía" style="display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border:1px dashed var(--line);border-radius:6px;cursor:pointer;font-size:15px;">
                 📷
                 <input type="file" accept="image/*" style="display:none" onchange="App.capturarFotoRevalidada('${group}',${i},this)"/>
               </label>`
        }
      </td>`
        : ""
    ...
    <td style="white-space:nowrap;">
      ${
        group === "pendientes" && (row.tipo === "previo" || row.tipo === "despacho" || row.tipo === "revalidada")
          ? `<button class="btn btn-primary btn-sm" style="margin-right:4px;" onclick="App.marcarPendienteEnProceso(${i})" title="Mover a &quot;Movimientos en proceso&quot; — todavía no sube a Previos/Despachos/Revalidadas">🔄 En proceso</button>`
          : ""
      }
      ${
        group === "enProceso" && (row.tipo === "previo" || row.tipo === "despacho" || row.tipo === "revalidada")
          ? `<button class="btn btn-primary btn-sm" style="margin-right:4px;" onclick="App.finalizarEnProceso(${i})" title="Sube de verdad al control de Previos/Despachos/Revalidadas que corresponda">✅ Finalizado</button>
             <button class="btn btn-ghost btn-sm" style="margin-right:4px;" onclick="App.regresarEnProcesoAPendiente(${i})" title="Regresar a Pendientes">↩️ Pendiente</button>`
          : ""
      }
      ${
        row._solicitudEliminacion
    ...
    ${(d.capturas || [])
      ...
      <div class="captura-item">
        ...
        ${
          (c.enProceso || []).length
            ? `
          <div class="mini-title">5. Movimientos en proceso (${(c.enProceso || []).length})</div>
          <div style="overflow-x:auto;">
          <table class="mini-table">
            <thead><tr>${EN_PROCESO_FIELDS.map((f) => `<th>${f.label}</th>`).join("")}</tr></thead>
            <tbody>${(c.enProceso || []).map((r) => `<tr>${EN_PROCESO_FIELDS.map((f) => `<td>${esc(r[f.k])}</td>`).join("")}</tr>`).join("")}</tbody>
          </table>
          </div>
        `
            : ""
        ...
        ${
          (c.otrasActividades || []).filter((a) => a && a.trim()).length
            ? `
          <div class="mini-title">6. Otras actividades</div>
    ...
    ${
      ...
      <div class="panel" style="margin-bottom:12px;">
        ...
        <div style="display:flex;gap:8px;align-items:center;margin-top:10px;flex-wrap:wrap;">
          ...
          <button class="row-del" onclick="App.eliminarPendienteGlobal('${item.fecha}','${esc(item.capturaId)}','${esc(
            (p.guia || "").trim()
          )}')" title="Eliminar este pendiente por completo">✕</button>
        </div>
      </div>`;
            })
            .join("")
...
const App = {
  async chooseAduana(aduana) {
    ...
    state.errorMsg = "";
    // Se muestra una pantalla de carga breve en vez de la de login de inmediato — antes,
    // la de login (con el botón de "Coordinador", que depende del catálogo) se dibujaba
    // ANTES de que terminara de cargar el catálogo de la aduana recién elegida, así que
    // por un instante se veía con los datos de la aduana anterior (o vacío) — el botón de
    // Coordinador podía "no aparecer" aunque sí hubiera nombres cargados, y solo se veía
    // bien hasta la segunda vez que se entraba, cuando la carga ya había terminado.
    state.view = "loadingAduana";
    render();
    try {
      if (navigator.onLine) {
        const cats = await fbLoadCatalogos(aduana);
        editableCats = sanitizeCats(cats || {});
      } else {
        const cached = loadLocalCache();
        editableCats = sanitizeCats(cached.catalogos || {});
      }
    } catch (e) {
      const cached = loadLocalCache();
      editableCats = sanitizeCats(cached.catalogos || {});
   ...
   * sin esperar a que el navegador dispare su propio evento de "regresó la señal" (que
   * a veces no se dispara bien en celulares). */
  ...
  async autorizarEliminacionDesdeGlobal(fecha, capturaId, grupo, tipo, marca) {
    ...
    if (tipo === "foto") {
      ...
      const guiaTrim = (row.guia || "").trim();
      c[grupo].splice(idx, 1);
      // Se registra la exclusión, mismo motivo que en deleteRow — para que la fusión
      // con el servidor al guardar no "recupere" este renglón recién autorizado para
      // borrar, pensando que es algo que esta sesión no conocía.
      if (guiaTrim) {
        if (!c._recienBorrados) c._recienBorrados = [];
        c._recienBorrados.push({ grupo, guia: guiaTrim });
      }
    }
    render();
    try {
  ...
  /** Elimina un pendiente por completo desde la pantalla global de Pendientes — solo
   * Admin/Coordinador tienen acceso a esta pantalla, así que se borra directo, sin pasar
   * por el sistema de solicitud de eliminación (ese es para cuando Trámite/Ejecutivo
   * quieren borrar algo). Si venía de un traslado automático (origenFecha), se marca el
   * original histórico como descartado, igual que al borrar desde dentro del reporte —
   * para que la reconciliación no lo vuelva a traer de regreso solo. */
  async eliminarPendienteGlobal(fecha, capturaId, guia) {
    if (!confirm(`¿Eliminar por completo el pendiente de la guía ${guia}? No se puede deshacer.`)) return;
    const day = state.reports[fecha];
    const captura = day && (day.capturas || []).find((c) => c.id === capturaId);
    if (!captura) return;
    const idx = (captura.pendientes || []).findIndex((p) => (p.guia || "").trim() === guia);
    if (idx === -1) return;
    const tramitadorActual = captura.tramitador;
    captura.pendientes.splice(idx, 1);
    // Se registra la exclusión, mismo motivo que en deleteRow — para que la fusión con
    // el servidor al guardar no "recupere" este pendiente pensando que es algo que
    // esta sesión no conocía.
    const guiaTrim = (guia || "").trim();
    if (guiaTrim) {
      if (!captura._recienBorrados) captura._recienBorrados = [];
      captura._recienBorrados.push({ grupo: "pendientes", guia: guiaTrim });
    }
    render();
    // Se busca en TODOS los días (no solo el que diga "origenFecha" en este renglón) y
    // se compara SOLO por guía — mismo criterio que al borrar desde dentro del reporte,
    // para no dejar sin marcar algún rastro viejo duplicado de la misma guía.
    if ((guia || "").trim()) {
      let cambiado = false;
      const diasParaGuardar = new Set();
      Object.values(state.reports).forEach((d) => {
        (d.capturas || []).forEach((c) => {
          if (!mismoNombre(c.tramitador, tramitadorActual)) return;
          (c.pendientes || []).forEach((p) => {
            if (p.bloqueado && !p.descartado && normalizarNombre(p.guia) === normalizarNombre(guia)) {
              p.descartado = true;
              cambiado = true;
              diasParaGuardar.add(d.fecha);
            }
          });
        });
      });
      if (cambiado) {
        for (const f of diasParaGuardar) {
          if (f === fecha) continue; // se guarda junto con el resto más abajo de todos modos
          try {
            await saveReportDay(f);
          } catch (e) {}
        }
      }
    }
    try {
      ...
      state.errorMsg = "Se eliminó aquí, pero no se pudo guardar en la nube (" + e.message + "). Vuelve a intentarlo con internet.";
      render();
    }
  },
  async reasignarPendienteGlobal(fecha, capturaId, guia, nuevoTramitador) {
    try {
      let tramitadorAnterior = null;
      // La mutación se aísla en una función aparte para poder repetirla una segunda vez
      // como red de seguridad más abajo — por si la sincronización en tiempo real trae
      // una actualización a medio camino (ej. el día nuevo ya se actualizó pero el
      // viejo todavía no) justo mientras se están guardando los dos días, lo cual podía
      // hacer que el pendiente se viera "duplicado" en ambos lados por un momento.
      const aplicarMutacion = () => {
        const day = state.reports[fecha];
        const captura = day && (day.capturas || []).find((c) => c.id === capturaId);
        if (!captura) return null;
        const idx = (captura.pendientes || []).findIndex((p) => (p.guia || "").trim() === guia);
        if (idx === -1) return null; // ya no está ahí — se quitó en un intento anterior
        const pend = captura.pendientes[idx];
        if (!nuevoTramitador || mismoNombre(nuevoTramitador, captura.tramitador)) return null; // sin cambio real
        tramitadorAnterior = captura.tramitador;
        captura.pendientes.splice(idx, 1);
        // Se registra qué se acaba de quitar de la persona anterior — mismo motivo que
        // al borrar: para que la fusión con el servidor al guardar no lo "recupere" en
        // la hoja vieja, pensando que es algo que esta sesión no conocía todavía. Sin
        // esto, el pendiente se quedaba viéndose en AMBAS personas a la vez.
        if (!captura._recienBorrados) captura._recienBorrados = [];
        captura._recienBorrados.push({ grupo: "pendientes", guia });
        const copia = { ...pend };
        ...
        normalizeCaptura(destino);
        const yaEnDestino = (destino.pendientes || []).some((p) => (p.guia || "").trim() === guia && p.origenFecha === copia.origenFecha);
        if (!yaEnDestino) destino.pendientes.push(copia);
        return hoy;
      };

      const hoy = aplicarMutacion();
      if (!hoy) return;
      render();

      // Si esta guía tenía una asignación de seguimiento a nombre de la persona
      // anterior, se actualiza también para que apunte a la nueva persona — si no se
      // hiciera esto, el registro de seguimiento se quedaría mostrando a alguien que
      // ya no la está trabajando, aunque el pendiente ya se haya movido bien.
      if (tramitadorAnterior) {
        const asigEntry = Object.entries(state.asignaciones || {}).find(
          ([, a]) => (a.guia || "").trim() === guia && mismoNombre(a.tramitador, tramitadorAnterior) && a.estatus === "pendiente"
        );
        if (asigEntry) {
          const [asigId, asig] = asigEntry;
          const actualizada = { ...asig, tramitador: nuevoTramitador };
          state.asignaciones[asigId] = actualizada;
          render();
          try {
            await fbSaveAsignacion(state.aduanaActiva, asigId, actualizada);
          } catch (e) {
            state.errorMsg = "El pendiente se reasignó bien, pero no se pudo actualizar la asignación de seguimiento en la nube (" + e.message + ").";
            render();
          }
        }
      }
      ...
      // Red de seguridad: se repite la misma mutación una vez más por si una
      // actualización en tiempo real reemplazó los datos justo a medio camino (por
      // ejemplo, justo al recuperar el acceso después de una interrupción de Firebase)
      // y la mutación original se perdió por completo, en vez de solo verse duplicada.
      // Si aplicarMutacion() dice que SÍ hizo falta aplicarla de nuevo (no fue un "ya
      // no hacía falta, todo bien"), hay que volver a guardar también — si no, la
      // corrección se queda solo viéndose en la pantalla, sin llegar a la nube.
      const hoyReaplicado = aplicarMutacion();
      render();
      if (hoyReaplicado) {
        try {
          await saveReportDay(fecha);
        ...
        if (hoyReaplicado !== fecha) {
          try {
            await saveReportDay(hoyReaplicado);
          } catch (e) {
            fallo.push(fmtDateHuman(hoyReaplicado));
          }
        }
        render();
      }
  ...
  async startManual() {
    try {
      ...
      state.errorMsg = "No se pudo crear el reporte nuevo (" + (e && e.message ? e.message : "error desconocido") + "). Avísale a soporte con este mensaje.";
      render();
    }
  },
  ...
  /** Mueve TODOS los pendientes de un mismo lote (los que llegaron juntos de una misma
   * carga masiva de asignaciones) a "En proceso" de un solo jalón — sin tener que
   * marcarlos uno por uno. Cualquier guía se puede sacar del lote antes con su ✕ normal
   * si necesita tratarse aparte; ya no se contará en el botón del lote una vez borrada. */
  marcarLoteEnProceso(loteId) {
    try {
      const c = normalizeCaptura(state.currentCaptura);
      const indices = [];
      (c.pendientes || []).forEach((row, i) => {
      ...
      indices.sort((a, b) => b - a).forEach((i) => App.marcarPendienteEnProceso(i));
      render();
    } catch (e) {
      state.errorMsg = "No se pudo mover el lote a En proceso (" + e.message + "). Avísale a soporte con este mensaje.";
      render();
    }
  },
  /** Primera etapa del nuevo flujo: mueve un pendiente a "Movimientos en proceso" — solo
   * traslada los datos tal cual (guía, cliente, almacén, ref, ejecutivo, pedimento,
   * observaciones), sin subirlo todavía a Previos/Despachos/Revalidadas. El número de
   * ocurrencia (1er/2do previo, etc.) se calcula hasta la segunda etapa (Finalizado), no
   * aquí — así el conteo siempre refleja lo que hay en el control final en ese momento,
   * no lo que había cuando se movió a "en proceso" (que puede quedar desactualizado si
   * pasa tiempo entre una etapa y la otra). */
  marcarPendienteEnProceso(i) {
    try {
      const c = normalizeCaptura(state.currentCaptura);
      ...
      const row = {};
      EN_PROCESO_FIELDS.forEach((f) => (row[f.k] = pend[f.k] !== undefined ? pend[f.k] : ""));
      if (pend._sector) row._sector = pend._sector; // viaja oculto, igual que ya hacía en Pendientes
      if (pend._loteId) row._loteId = pend._loteId;
      if (pend.origenFecha) row.origenFecha = pend.origenFecha; // rastro para que la reconciliación lo encuentre aquí

      c.enProceso.push(row);
      c.pendientes.splice(i, 1);
      // Se registra qué se acaba de quitar de Pendientes — mismo motivo que al borrar:
      // para que la fusión con el servidor al guardar no lo "recupere" pensando que es
      // algo que esta sesión no conocía todavía.
      if ((pend.guia || "").trim()) {
        if (!c._recienBorrados) c._recienBorrados = [];
        c._recienBorrados.push({ grupo: "pendientes", guia: (pend.guia || "").trim() });
      }
      render();
    } catch (e) {
      state.errorMsg = "No se pudo mover el pendiente a En proceso (" + e.message + "). Avísale a soporte con este mensaje.";
      render();
    }
  },
  /** Regresa un renglón de "En proceso" de vuelta a Pendientes — por si se movió por
   * error, o si de plano no se va a poder terminar y hay que dejarlo como pendiente de
   * nuevo. No borra nada, solo lo regresa a la sección anterior. */
  regresarEnProcesoAPendiente(i) {
    try {
      const c = normalizeCaptura(state.currentCaptura);
      const item = c.enProceso[i];
      if (!item) return;
      const row = {};
      PENDIENTE_FIELDS.forEach((f) => (row[f.k] = item[f.k] !== undefined ? item[f.k] : ""));
      if (item._sector) row._sector = item._sector;
      if (item._loteId) row._loteId = item._loteId;
      if (item.origenFecha) row.origenFecha = item.origenFecha;
      c.pendientes.push(row);
      c.enProceso.splice(i, 1);
      if ((item.guia || "").trim()) {
        if (!c._recienBorrados) c._recienBorrados = [];
        c._recienBorrados.push({ grupo: "enProceso", guia: (item.guia || "").trim() });
      }
      render();
    } catch (e) {
      state.errorMsg = "No se pudo regresar a Pendientes (" + e.message + "). Avísale a soporte con este mensaje.";
      render();
    }
  },
  /** Segunda etapa del nuevo flujo: sube de verdad un renglón de "En proceso" al control
   * que corresponda (Previos/Despachos/Guías revalidadas) — aquí es donde se calcula el
   * número de ocurrencia (1er/2do previo, etc.), con exactamente el mismo criterio de
   * siempre: solo cuenta las veces que ESTE MISMO tramitador ya hizo esa guía en el
   * control final, no un conteo global de todos los reportes (para no castigar con
   * "2do previo" a alguien que la está haciendo por primera vez, solo porque alguien
   * más ya la trabajó antes). */
  finalizarEnProceso(i) {
    try {
      const c = normalizeCaptura(state.currentCaptura);
      const item = c.enProceso[i];
      if (!item) return;
      if (item.tipo !== "previo" && item.tipo !== "despacho" && item.tipo !== "revalidada") return;
      ...
      row.ref = item.ref || "";
      if (tipo === "previo" || tipo === "despacho") row.ejecutivo = item.ejecutivo || "";
      if (tipo === "previo" && item._sector) row.sector = item._sector;
      if (tipo === "despacho") row.pedimento = item.pedimento || "";
      if ((tipo === "previo" || tipo === "despacho") && item.observaciones) {
        row.observaciones = item.observaciones;
        row.observadoPor = item.observadoPor || "";
      }
      ...
      if (item.origenFecha) row.origenFecha = item.origenFecha; // rastro para que la reconciliación lo encuentre aquí

      c[grupo].push(row);
      c.enProceso.splice(i, 1);
      if (guia) {
        if (!c._recienBorrados) c._recienBorrados = [];
        c._recienBorrados.push({ grupo: "enProceso", guia });
      }
      render();
    } catch (e) {
      state.errorMsg = "No se pudo finalizar el movimiento (" + e.message + "). Avísale a soporte con este mensaje.";
      render();
    }
  },
  addRow(group) {
  ...
  async deleteRow(group, i) {
    ...
    const debeMarcarOriginalComoDescartado = group === "pendientes" && !row.bloqueado;
    const tramitadorActual = state.currentCaptura.tramitador;
    const guiaRow = (row.guia || "").trim();
    state.currentCaptura[group].splice(i, 1);
    // Se registra qué se acaba de borrar a propósito — para que, al guardar, la fusión
    // con el servidor (ver fusionarConServidor) no lo "recupere" pensando que es algo
    // que esta sesión simplemente no conocía todavía.
    if (guiaRow) {
      if (!state.currentCaptura._recienBorrados) state.currentCaptura._recienBorrados = [];
      state.currentCaptura._recienBorrados.push({ grupo: group, guia: guiaRow });
    }
    render();
    if (debeMarcarOriginalComoDescartado && guiaRow) {
      // Se busca en TODOS los días (no solo el que diga "origenFecha" en este renglón)
      // y se compara SOLO por guía — no también por fecha de origen exacta — porque
      // puede haber más de un rastro viejo de la misma guía (por ejemplo, restos de
      // antes de tener este sistema bien afinado), y dejar cualquiera sin marcar
      // significaría que ese solo baste para que la guía se vuelva a traer de regreso.
      let cambiado = false;
      const diasParaGuardar = new Set();
      Object.values(state.reports).forEach((day) => {
        (day.capturas || []).forEach((c) => {
          if (!mismoNombre(c.tramitador, tramitadorActual)) return;
          (c.pendientes || []).forEach((p) => {
            if (p.bloqueado && !p.descartado && normalizarNombre(p.guia) === normalizarNombre(guiaRow)) {
              p.descartado = true;
              cambiado = true;
              diasParaGuardar.add(day.fecha);
            }
    ...
    // Esto es lo que realmente faltaba: guardar el propio renglón borrado. Lo de
    // arriba solo marca como "descartado" el rastro histórico en OTROS días — pero si
    // no se guarda también ESTA hoja (donde de verdad se quitó el renglón), el borrado
    // nunca llega a la nube. Se veía desaparecer un instante en pantalla, pero en
    // cuanto se recargaran los datos (o se volviera a abrir este mismo reporte), el
    // renglón seguía estando ahí en el servidor y "regresaba" solo.
    await guardarCambioInmediatoSiExiste();
  },
  ...
  async capturarFotoRevalidada(group, i, inputEl) {
    ...
    if (!file) {
      // Puede pasar en algunos celulares que, al regresar de la cámara, el navegador
      // no entregue la foto a la primera — se avisa claro en vez de no hacer nada, para
      // que se sepa que hay que volver a intentarlo, en vez de pensar que ya se guardó.
      state.errorMsg = "No se detectó ninguna foto — puede que el celular haya tardado en regresar de la cámara. Vuelve a intentarlo.";
      render();
      return;
    }
    const LIMITE_MB = 20;
  ...
  async confirmSave() {
    try {
      ...
      // Si la fecha cambió pero el día viejo no quedó vacío (había más hojas ese día),
      // hay que borrar explícitamente el nodo viejo de ESTA hoja en particular — un
      // guardado normal ya no reescribe el día completo, así que no lo quita solo.
      if (wasEditing && state.editingFecha !== c.fecha && state.reports[state.editingFecha]) {
        try {
          if (navigator.onLine) await fbDeleteCapturaUnica(state.aduanaActiva, state.editingFecha, c.id);
        } catch (e) {}
      }
      await saveReportDay(c.fecha);
  ...
  async deleteCaptura(fecha, capturaId) {
    ...
    if (state.reports[fecha].capturas.length === 0) {
      ...
      // Se borra específicamente ESA hoja de Firebase (no solo se re-guardan las
      // demás) — un guardado normal ya no reescribe el día completo, así que hay que
      // decirle explícitamente que quite ese nodo puntual.
      try {
        if (navigator.onLine) await fbDeleteCapturaUnica(state.aduanaActiva, fecha, capturaId);
      } catch (e) {}
    }
    persistLocalCache();
    render();
...
window.addEventListener("error", (e) => {
  if (state.view === "login" || state.view === "selectAduana" || state.view === "loadingAduana") return; // no molestar en pantallas de entrada
  state.errorMsg = "Ocurrió un error inesperado (" + (e.message || "desconocido") + "). Si vuelve a pasar, avísale a soporte.";
  render();
});
// Mismo respaldo que arriba, pero para errores dentro de funciones async (ej. algo que
// truena adentro de un "await") — sin esto, esos errores fallaban en TOTAL SILENCIO: no
// pasaba nada visible, ni un mensaje, ni el botón parecía hacer algo, como si la app se
// hubiera "colgado" sin ninguna pista de qué pasó.
window.addEventListener("unhandledrejection", (e) => {
  if (state.view === "login" || state.view === "selectAduana" || state.view === "loadingAduana") return; // no molestar en pantallas de entrada
  const msg = e.reason && e.reason.message ? e.reason.message : String(e.reason || "desconocido");
  state.errorMsg = "Ocurrió un error inesperado (" + msg + "). Si vuelve a pasar, avísale a soporte.";
  render();
});
...
// Reportes y asignaciones ya no se refrescan por intervalo — se escuchan en tiempo real
// desde que se inicia sesión (ver iniciarEscuchaEnTiempoReal). El intervalo de arriba
// (cada 20s) ya cubre el reintento de subir lo pendiente por su cuenta.
