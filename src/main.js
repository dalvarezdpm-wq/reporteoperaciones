...
const ADMIN_PASSWORDS = { GDL: "ow2026", TOLUCA: "OWTOLUCA" };
...
function sanitizeCats(cats) {
  ...
  const base = { clientes: [], almacenes: [], tramitadores: esGDL ? [...TRAMITADORES] : [], ejecutivos: esGDL ? [...EJECUTIVOS] : [] };
  const out = { ...base, ...(cats || {}) };
...
async function backgroundRefresh() {
  ...
  // Mientras la cámara del escáner de código de barras está abierta, NO se redibuja la
  // pantalla — hacerlo destruiría el elemento <video> a medio escaneo y cortaría la cámara.
...
// Nada de esto se guarda en `state` porque no son datos serializables para volver a
// dibujar la pantalla — son solo handles en memoria para controlar la cámara mientras
// el escáner está abierto.
let _scannerStream = null;
let _scannerInterval = null;
let _scannerTorchTrack = null;
let _scannerDecodeCtx = null; // { videoEl, codeReader, canvas, ctx } — para poder reanudar el escaneo tras "Escanear de nuevo" sin reabrir la cámara

async function iniciarCamaraScanner() {
  const videoEl = document.getElementById("scannerVideo");
  if (!videoEl) return;
  // Se intenta primero con la mejor calidad posible (resolución alta + enfoque continuo si el
  // celular lo soporta) — un código de barras chico se lee mal en baja resolución. Si el
  // dispositivo no soporta esas opciones exactas, se reintenta con algo más simple en vez
  ...
  if (!stream) {
    state.scannerError = "No se pudo acceder a la cámara (" + (ultimoError && ultimoError.message ? ultimoError.message : "revisa que hayas dado permiso") + ").";
    render();
    return;
  }
  ...
  // Botón de linterna, si el dispositivo la soporta — sin volver a dibujar toda la pantalla
  // (eso destruiría el <video> a medio escaneo), solo se ajusta ese botón directo en el DOM.
  try {
    const track = stream.getVideoTracks()[0];
    const caps = track.getCapabilities ? track.getCapabilities() : {};
    if (caps && caps.torch) {
      _scannerTorchTrack = track;
  ...
  const canvas = document.createElement("canvas"); // el tamaño se ajusta solo, cuadro a cuadro, a la resolución real de la cámara
  const ctx = canvas.getContext("2d");
  _scannerDecodeCtx = { videoEl, codeReader, canvas, ctx };

  iniciarLoopDecodificacion();
}

/** Las guías de FedEx son puramente numéricas — normalmente 12 dígitos (Express/Ground),
 * a veces 15 (Ground Economy/antes SmartPost), y hasta 20-22 en casos especiales. Cualquier
 * otra cosa (letras, muy corto, muy largo) casi seguro NO es la guía sino otro código de la
 * misma hoja (ej. número de cuenta, código postal) — se descarta sola sin molestar con una
 * confirmación de algo que claramente no es lo que se busca. */
function esProbableGuiaFedEx(texto) {
  return /^\d{10,22}$/.test((texto || "").trim());
}

/** El ciclo que realmente busca el código, cuadro por cuadro. Separado de
 * iniciarCamaraScanner() para poder reanudarlo después de "Escanear de nuevo" sin
 * tener que volver a pedir permiso de cámara ni reabrir el video desde cero. */
function iniciarLoopDecodificacion() {
  if (!_scannerDecodeCtx) return;
  const { videoEl, codeReader, canvas, ctx } = _scannerDecodeCtx;
  _scannerInterval = setInterval(() => {
    if (!videoEl.videoWidth || !videoEl.videoHeight) return;
    // Se decodifica el cuadro de video COMPLETO, tal como llega de la cámara — sin recortar
    // ninguna zona. (Antes se intentó recortar solo el centro para "acercar" digitalmente,
    // pero esa cuenta se hacía sobre los píxeles crudos de la cámara sin tomar en cuenta que
    // la pantalla ya recorta y reescala esa imagen para que quepa en el celular — el
    // recuadro azul que se ve en pantalla terminaba sin corresponder con la zona que
    // realmente se analizaba, y por eso dejaba de detectar aunque se encuadrara bien.)
    ...
    try {
      ...
      if (result) {
        ...
        if (!esProbableGuiaFedEx(texto)) {
          // Detectó un código, pero no tiene forma de guía (ej. otro código de la misma
          // hoja) — se avisa un instante en la pantalla y se sigue buscando sin pausar
          // ni pedir confirmación de algo que claramente no es lo que se necesita.
          const aviso = document.getElementById("scannerAvisoDescartado");
          if (aviso) {
            aviso.textContent = `Se detectó "${texto}" pero no parece una guía — se ignora, sigue buscando…`;
        ...
        // Se PAUSA el escaneo (sin apagar la cámara) y se pide confirmar antes de meter
        // el número al campo — así, si detectó otro código por error (ej. otro código de
        // la misma hoja) o una lectura corrupta, la persona lo nota antes de que se use,
        // en vez de descubrirlo hasta después ya con el dato equivocado en el reporte.
        if (_scannerInterval) {
          clearInterval(_scannerInterval);
          _scannerInterval = null;
        }
        mostrarConfirmacionScanner(texto);
      }
    } catch (e) {
      // Que no encuentre nada en un intento (NotFoundException) es normal — se ignora
      // y se vuelve a intentar en el siguiente cuadro, cada poco menos de un tercio de segundo.
    }
  }, 280);
}

function mostrarConfirmacionScanner(texto) {
  const box = document.getElementById("scannerConfirmBox");
  const txt = document.getElementById("scannerConfirmText");
  const btnOk = document.getElementById("scannerConfirmBtn");
  const btnRetry = document.getElementById("scannerRetryBtn");
  if (!box || !txt || !btnOk || !btnRetry) return;
  txt.textContent = texto;
  box.style.display = "block";
  btnOk.onclick = () => {
    detenerCamaraScanner();
    App.aplicarResultadoScanner(texto);
  };
  btnRetry.onclick = () => {
    box.style.display = "none";
    iniciarLoopDecodificacion(); // reanuda el mismo stream de cámara, sin pedir permiso otra vez
  };
}
...
function detenerCamaraScanner() {
  ...
  _scannerTorchTrack = null;
  _scannerDecodeCtx = null;
}
...
/** Ventana de cámara de pantalla completa para escanear el código de barras de una guía.
 * Se dibuja encima de TODO (incluso encima del topbar y el contenido normal), como una capa
 * fija, para que funcione sin importar en qué pantalla se abrió (captura, asignaciones). */
function scannerModalHtml() {
  if (!state.scannerActive) return "";
  return `
  <div style="position:fixed;inset:0;background:#000;z-index:9999;display:flex;flex-direction:column;">
    <div style="display:flex;justify-content:space-between;align-items:center;padding:14px 16px;background:rgba(0,0,0,.75);">
      <div style="color:#fff;font-weight:600;font-size:14px;">📷 Escanea el código de la guía</div>
      <button onclick="App.cerrarScanner()" style="background:none;border:none;color:#fff;font-size:24px;cursor:pointer;line-height:1;padding:4px 8px;">✕</button>
    </div>
    <div style="flex:1;position:relative;overflow:hidden;display:flex;align-items:center;justify-content:center;">
      <video id="scannerVideo" style="width:100%;height:100%;object-fit:cover;" playsinline muted autoplay></video>
      <div id="scannerAvisoDescartado" style="display:none;position:absolute;top:14px;left:16px;right:16px;background:rgba(184,114,30,.92);color:#fff;font-size:12px;text-align:center;padding:8px 12px;border-radius:8px;"></div>
      <div style="position:absolute;border:3px solid #3F7FC1;border-radius:10px;width:82%;max-width:460px;height:34vh;max-height:160px;box-shadow:0 0 0 2000px rgba(0,0,0,.4);pointer-events:none;"></div>
      <div id="scannerConfirmBox" style="display:none;position:absolute;left:16px;right:16px;bottom:16px;">
        <div style="background:#fff;border-radius:12px;padding:16px;text-align:center;box-shadow:0 4px 20px rgba(0,0,0,.4);">
          <div style="font-size:11.5px;color:var(--muted);text-transform:uppercase;letter-spacing:.04em;margin-bottom:4px;">¿Es correcto este código?</div>
          <div id="scannerConfirmText" style="font-size:21px;font-weight:700;color:var(--navy);margin-bottom:14px;word-break:break-all;"></div>
          <div style="display:flex;gap:8px;">
            <button id="scannerRetryBtn" class="btn btn-ghost" style="flex:1;">🔁 Escanear de nuevo</button>
            <button id="scannerConfirmBtn" class="btn btn-primary" style="flex:1;">✅ Sí, usar</button>
          </div>
        </div>
      </div>
    </div>
    <div style="padding:16px;background:rgba(0,0,0,.75);text-align:center;">
      ${
        state.scannerError
          ? `<div style="color:#FF8A80;font-size:13px;margin-bottom:10px;">${esc(state.scannerError)}</div>`
          : `<div style="color:#fff;font-size:12.5px;margin-bottom:10px;">Centra el código de barras de la guía dentro del recuadro, a unos 10-15 cm de distancia</div>`
      }
      <div style="display:flex;gap:10px;justify-content:center;">
        <button id="scannerTorchBtn" onclick="App.toggleTorch()" style="display:none;align-items:center;gap:6px;border:none;background:#fff;border-radius:8px;padding:10px 16px;font-weight:600;cursor:pointer;">🔦 Linterna</button>
        <button class="btn btn-ghost" style="background:#fff;" onclick="App.cerrarScanner()">Cancelar</button>
      </div>
    </div>
  </div>`;
}
  ...
  return `<tr>
    ${fields
      .map((f) => {
        ...
        if (f.k === "guia") {
          return `<td style="white-space:nowrap;"><div style="display:flex;gap:4px;align-items:center;">
            <input${dl} value="${esc(row[f.k])}" oninput="App.updateRow('${group}',${i},'${f.k}',this.value)"${blur} style="min-width:90px;"/>
            <button type="button" title="Escanear código de barras de la guía" onclick="App.abrirScanner('${group}',${i})" style="flex-shrink:0;border:1px solid var(--line);background:var(--panel);border-radius:6px;padding:4px 7px;cursor:pointer;font-size:14px;line-height:1;">📷</button>
          </div></td>`;
        }
  ...
  return `
    ...
    <div class="panel" style="margin-bottom:20px;">
      <div class="field-grid">
        ...
        <div class="field"><label>Guía</label>
          <div style="display:flex;gap:6px;align-items:center;">
            <input id="asig_guia" placeholder="Número de guía" value="${esc(state.asigDraft.guia)}" oninput="App.updateAsigDraft('guia',this.value)" style="flex:1;"/>
            <button type="button" title="Escanear código de barras de la guía" onclick="App.abrirScannerAsignacion()" style="flex-shrink:0;border:1px solid var(--line);background:var(--panel);border-radius:6px;padding:8px 10px;cursor:pointer;font-size:15px;line-height:1;">📷</button>
          </div>
        </div>
...
const App = {
  ...
  goAsignaciones() {
    ...
    state.asigDraft = {
      ...
      sector: "",
      ejecutivo: "",
      tramitador: (editableCats.tramitadores || [])[0] || "",
    };
    render();
  },
  ...
  async crearAsignacion() {
    ...
    state.asigDraft = { tipo: "previo", ref: "", guia: "", cliente: "", almacen: "", sector: "", ejecutivo: "", tramitador };
    render();
    try {
   ...
   * quedan ligadas a un solo tramitador). */
  startManual() {
    if (state.userRole === "tramite") {
      const hoy = todayStr();
      const day = state.reports[hoy];
      const idxExistente = day && day.capturas ? day.capturas.findIndex((c) => (c.tramitador || "").trim() === state.user) : -1;
      if (idxExistente !== -1) {
  ...
  /** Abre la cámara para escanear el código de barras de una guía dentro de un renglón
   * del reporte (previos/despachos/revalidadas/pendientes). */
  abrirScanner(group, idx) {
    state.scannerTarget = { kind: "row", group, idx };
    state.scannerActive = true;
    state.scannerError = "";
    render();
    iniciarCamaraScanner();
  },
  /** Igual, pero para el campo Guía del formulario de "Crear asignación". */
  abrirScannerAsignacion() {
    state.scannerTarget = { kind: "asignacion" };
    state.scannerActive = true;
    state.scannerError = "";
    render();
    iniciarCamaraScanner();
  ...
  /** Prende/apaga la linterna del celular mientras se escanea. No vuelve a dibujar toda
   * la pantalla (eso destruiría el <video> a medio escaneo) — solo cambia el texto del
   * botón directo en el DOM. */
  async toggleTorch() {
    if (!_scannerTorchTrack) return;
    try {
      state.scannerTorchOn = !state.scannerTorchOn;
      await _scannerTorchTrack.applyConstraints({ advanced: [{ torch: state.scannerTorchOn }] });
      const btn = document.getElementById("scannerTorchBtn");
      if (btn) btn.textContent = state.scannerTorchOn ? "🔦 Apagar" : "🔦 Linterna";
    } catch (e) {}
  },
  /** Se llama automáticamente en cuanto ZXing detecta un código de barras válido —
   * rellena el campo Guía correspondiente y cierra la cámara sola. */
  aplicarResultadoScanner(texto) {
    const target = state.scannerTarget;
    state.scannerActive = false;
    state.scannerTarget = null;
    state.scannerError = "";
    if (!texto) {
    ...
    if (target && target.kind === "row") {
      const row = state.currentCaptura[target.group] && state.currentCaptura[target.group][target.idx];
      if (row) row.guia = texto;
      render();
      App.autocompletarRefPorGuia(target.group, target.idx); // mismo autocompletado de Ref. que al salir del campo a mano
    } else if (target && target.kind === "asignacion") {
      state.asigDraft.guia = texto;
      render();
    } else {
      render();
    }
