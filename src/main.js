      ...
      lista.length === 0
        ...
        : lista
          ...
          <div style="font-weight:600;font-size:12.5px;">
            <span class="pill ${a.tipo === "previo" ? "pill-navy" : "pill-verde"}">${esc(a.tipo)}</span> Guía ${esc(a.guia)}
          </div>
...
function statsDiariasPorTramitador(desde, hasta) {
  ...
  allCapturas()
    ...
    .forEach((c) => {
      const t = (c.tramitador || "").trim() || "Sin asignar";
      const key = c.fecha + "||" + t;
      if (!map[key]) map[key] = { fecha: c.fecha, nombre: t, previos: 0, despachos: 0, pendientes: 0 };
      map[key].previos += (c.previos || []).length;
      map[key].despachos += (c.despachos || []).length;
      map[key].pendientes += (c.pendientes || []).length;
    });
  return Object.values(map)
    .map((r) => ({ ...r, total: r.previos + r.despachos + r.pendientes }))
...
function viewKPIs() {
  ...
  return `
    ...
    <div class="panel" style="overflow-x:auto;">
      ...
      <table class="mini-table" style="min-width:560px;">
        <thead><tr><th>Fecha</th><th>Tramitador</th><th>Previos</th><th>Despachos</th><th>Pendientes</th><th>Total</th></tr></thead>
        <tbody>
        ${detalle
          .map(
            (r) => `<tr>
          ...
          <td>${r.previos}</td><td>${r.despachos}</td><td>${r.pendientes}</td>
...
const App = {
  ...
  setKpiPreset(preset) {
    ...
    if (preset === "hoy") {
      ...
      state.kpiDesde = d.toISOString().slice(0, 10);
      state.kpiHasta = hoy;
    } else if (preset === "mes") {
    ...
    try {
      ...
      if (guia) {
        allCapturas().forEach((cap) => {
          ...
          const filas = grupo === "previos" ? cap.previos || [] : cap.despachos || [];
          filas.forEach((r) => {
