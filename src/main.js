...
let state = {
  ...
  asigMasivaDraft: { tipo: "previo", sector: "", tramitador: "" },
  asigMasivaMsg: "",
  asigEditId: null,
    ...
    ${fields
      .map((f) => {
        if (f.k === "almacen" && almacenFijo()) {
          return `<td style="color:var(--muted);">${esc(almacenFijo())} <span title="Fijo para esta aduana" style="font-size:10px;">🔒</span></td>`;
        }
        if (f.select) {
    ...
    <div class="panel" style="margin-bottom:20px;">
      <div class="field-grid">
        ...
        <div class="field"><label>Almacén</label>${
          almacenFijo()
            ? `<div style="padding:9px 10px;border:1px solid var(--line);border-radius:7px;color:var(--muted);background:#F4F6F5;">${esc(
                almacenFijo()
              )} 🔒</div>`
      ...
      <div style="margin-top:14px;">
        ...
        <div class="field-grid">
          <div class="field">
            ...
            <textarea id="asig_masiva_guias" rows="10" placeholder="875782762327&#10;861234567890&#10;745609590220" style="width:100%;font-family:'IBM Plex Mono',monospace;font-size:12.5px;padding:8px;border:1px solid var(--line);border-radius:var(--radius);resize:vertical;"></textarea>
          </div>
          <div class="field">
            <label>Clientes (opcional)</label>
            <textarea id="asig_masiva_clientes" rows="10" placeholder="IMPRO&#10;BRP MEXICANA&#10;CPQ" style="width:100%;font-family:'IBM Plex Mono',monospace;font-size:12.5px;padding:8px;border:1px solid var(--line);border-radius:var(--radius);resize:vertical;"></textarea>
          </div>
          <div class="field">
            <label>Referencia (opcional)</label>
            <textarea id="asig_masiva_refs" rows="10" placeholder="GLSI262480" style="width:100%;font-family:'IBM Plex Mono',monospace;font-size:12.5px;padding:8px;border:1px solid var(--line);border-radius:var(--radius);resize:vertical;"></textarea>
          </div>
          <div class="field">
            <label>Ejecutivo (opcional)</label>
            <textarea id="asig_masiva_ejecutivos" rows="10" placeholder="Alberto Pichardo" style="width:100%;font-family:'IBM Plex Mono',monospace;font-size:12.5px;padding:8px;border:1px solid var(--line);border-radius:var(--radius);resize:vertical;"></textarea>
          </div>
        </div>
...
const App = {
  ...
  goAsignaciones() {
    ...
    state.asigMasivaMsg = "";
    state.asigMasivaDraft = { tipo: "previo", sector: "", tramitador: "" };
    render();
  },
  goPendientesGlobal() {
    if (state.userRole !== "admin") return;
  ...
  async crearAsignacionesMasivas() {
    const elGuias = document.getElementById("asig_masiva_guias");
    const elClientes = document.getElementById("asig_masiva_clientes");
    const elRefs = document.getElementById("asig_masiva_refs");
    const elEjecutivos = document.getElementById("asig_masiva_ejecutivos");
    if (!elGuias) return;
    const partir = (texto) =>
      (texto || "")
        .split("\n")
        .map((l) => l.trim())
        .filter(Boolean);
    const guias = partir(elGuias.value);
    const clientes = partir(elClientes ? elClientes.value : "");
    const refs = partir(elRefs ? elRefs.value : "");
    const ejecutivos = partir(elEjecutivos ? elEjecutivos.value : "");
    if (guias.length === 0) {
    ...
    elGuias.value = "";
    if (elClientes) elClientes.value = "";
    if (elRefs) elRefs.value = "";
    if (elEjecutivos) elEjecutivos.value = "";
    const avisos = [];
