...
// El "controls" que regresa ZXing no se guarda en `state` porque no es un dato serializable
// para volver a dibujar la pantalla — es solo un handle en memoria para poder apagar la
// cámara cuando se cierra el escáner o cuando ya se detectó un código.
...
async function iniciarCamaraScanner() {
  ...
  try {
    ...
    // "environment" pide la cámara trasera en celulares/tablets (mejor para escanear que la frontal).
    const constraints = { video: { facingMode: { ideal: "environment" } } };
    _scannerControls = await codeReader.decodeFromConstraints(constraints, videoEl, (result) => {
      if (result) {
        const texto = (result.getText() || "").trim();
        detenerCamaraScanner();
        App.aplicarResultadoScanner(texto);
      }
      // Nota: ZXing llama a este callback en cada cuadro de video, incluso cuando no
      // detecta nada (con un error "NotFoundException" normal) — eso no es un problema,
      // solo lo ignoramos y seguimos esperando a que sí se detecte un código.
    });
  } catch (e) {
    state.scannerError = "No se pudo acceder a la cámara (" + (e && e.message ? e.message : "revisa que hayas dado permiso") + ").";
    render();
  }
}
...
function scannerModalHtml() {
  ...
  <div style="position:fixed;inset:0;background:#000;z-index:9999;display:flex;flex-direction:column;">
    ...
    <div style="flex:1;position:relative;overflow:hidden;display:flex;align-items:center;justify-content:center;">
      ...
      <div style="position:absolute;border:3px solid #3F7FC1;border-radius:10px;width:80%;max-width:440px;height:130px;box-shadow:0 0 0 2000px rgba(0,0,0,.4);pointer-events:none;"></div>
    </div>
    <div style="padding:16px;background:rgba(0,0,0,.75);text-align:center;">
      ${
        state.scannerError
          ...
          : `<div style="color:#fff;font-size:12.5px;margin-bottom:10px;">Centra el código de barras de la guía dentro del recuadro</div>`
      }
