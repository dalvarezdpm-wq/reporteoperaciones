...
async function iniciarCamaraScanner() {
  ...
  const ZOOM_W = 1000,
    ZOOM_H = 340; // resolución "digital" ampliada del recorte, para que se lea bien aunque el código se vea chico dentro de toda la imagen de la cámara
  canvas.width = ZOOM_W;
  canvas.height = ZOOM_H;
  const ctx = canvas.getContext("2d");

  _scannerInterval = setInterval(() => {
    if (!videoEl.videoWidth || !videoEl.videoHeight) return;
    // Se recorta la misma zona central que marca el recuadro guía en pantalla (ancho amplio,
    // franja angosta) y se dibuja AMPLIADA sobre un canvas aparte — así, aunque el código se
    // vea chico dentro de toda la imagen (ej. si alguien encuadra la hoja completa en vez de
    // acercarse solo al código), lo que realmente se manda a leer ya está "acercado"
    // digitalmente, con mucho más detalle por raya del código.
    ...
    const sx = (vw - cropW) / 2;
    const sy = (vh - cropH) / 2;
    ctx.drawImage(videoEl, sx, sy, cropW, cropH, 0, 0, ZOOM_W, ZOOM_H);
    try {
