...
async function backgroundRefresh() {
  ...
  try {
    ...
    notificarCambiosAsignaciones(state.asignaciones, nuevasAsignaciones);
 ...
 */
function notificarCambiosAsignaciones(anteriores, nuevas) {
  ...
  Object.entries(nuevas || {}).forEach(([id, a]) => {
    ...
    } else if (antes.estatus === "pendiente" && a.estatus === "completada" && state.userRole === "admin") {
...
function render() {
  try {
    if (state.view === "login") root.innerHTML = viewLogin();
      ...
      root.innerHTML =
        topbar() +
        `<div class="content">${notifPanelHtml()}${
          state.loading
            ? loadingBlock()
            : state.view === "home"
            ? viewHome()
