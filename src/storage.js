import { db, ref, get, set, remove, onValue, off } from "./firebase.js";
...
/** Convierte lo que venga crudo de Firebase para "reportes" (con "capturas" en
 * cualquiera de los dos formatos posibles) a la forma que espera el resto de la app:
 * un objeto por fecha, cada uno con "capturas" siempre como arreglo. */
function normalizarReportesCrudos(raw) {
  const out = {};
  if (!raw) return out;
...
export async function loadReports(aduana) {
  ...
  return snap.exists() ? normalizarReportesCrudos(snap.val()) : {};
}

/** Se queda "escuchando" los reportes de una aduana en tiempo real — a diferencia de
 * loadReports (que pregunta una sola vez), esto avisa SOLO cuando algo cambia de
 * verdad en el servidor (alguien más guarda, borra, etc.), sin tener que estar
 * preguntando cada cierto tiempo. Llama a "callback" con los reportes ya normalizados
 * cada vez que hay un cambio real — incluida la primera vez, con lo que ya exista.
 * Si la escucha misma llega a fallar (ej. problema de conexión persistente), se avisa
 * a través de "onError" en vez de fallar en silencio sin que nadie se entere.
 * Regresa una función para dejar de escuchar (llamarla al cambiar de aduana o salir). */
export function subscribeToReports(aduana, callback, onError) {
  const nodo = ref(db, base(aduana) + "/reportes");
  const manejador = (snap) => callback(snap.exists() ? normalizarReportesCrudos(snap.val()) : {});
  onValue(nodo, manejador, (error) => {
    if (onError) onError(error);
  });
  return () => off(nodo, "value", manejador);
}

/** Igual que subscribeToReports, pero para las asignaciones de una aduana. */
export function subscribeToAsignaciones(aduana, callback, onError) {
  const nodo = ref(db, base(aduana) + "/asignaciones");
  const manejador = (snap) => callback(snap.exists() ? snap.val() : {});
  onValue(nodo, manejador, (error) => {
    if (onError) onError(error);
  });
  return () => off(nodo, "value", manejador);
...
/** Trae el contenido actual (en el servidor, ahora mismo) de UNA sola hoja específica,
 * por su ID — para poder comparar/fusionar antes de sobrescribirla (ver saveReportDay
 * en main.js, que la usa para no perder renglones que otra sesión de la MISMA persona
 * -celular y computadora abiertos a la vez, por ejemplo- haya agregado mientras tanto).
 * Regresa null si esa hoja no existe todavía en el servidor. */
export async function loadCapturaUnica(aduana, fecha, capturaId) {
  const rutaDia = base(aduana) + "/reportes/" + fecha;
  const snap = await get(ref(db, rutaDia + "/capturas"));
  if (!snap.exists()) return null;
  const val = snap.val();
  if (Array.isArray(val)) {
    return val.find((c) => c && c.id === capturaId) || null;
  }
  return val[capturaId] || null;
}
