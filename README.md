...
- **Al reasignar un pendiente, la asignación de seguimiento relacionada también se actualiza a la nueva persona.** Antes, solo se movía el pendiente en el reporte, pero el registro de seguimiento (la "asignación" original) se quedaba mostrando a la persona anterior — ahora, si esa guía tenía una asignación pendiente a nombre de quien se la quitaste, se actualiza automáticamente para que apunte a la nueva persona.

- **Revisión general tras los fixes de hoy — encontrado un hueco más en el mismo problema.** El respaldo de sondeo (el que entra en acción solo si la sincronización en tiempo real llegara a fallar) tenía la misma vulnerabilidad: podía reemplazar un día que se estuviera guardando justo en ese momento. Ya tiene la misma protección que la escucha en tiempo real. Se revisaron también todos los demás lugares que quitan renglones de un reporte (borrar, reasignar, mover a en proceso, finalizar, eliminar actividades, borrar una hoja completa) — todos están correctos, sin ningún hueco pendiente encontrado.

- **Encontrada y corregida una condición de carrera que podía perder datos por completo — más probable justo después de una reconexión (como la interrupción de Firebase de hoy).** Si algo se mutaba en memoria y, antes de que terminara de guardarse, llegaba una actualización en tiempo real que reemplazaba todos los datos de golpe, el guardado terminaba actuando sobre una versión fresca que nunca tuvo el cambio — perdiéndolo por completo, sin ningún aviso ni duplicado, simplemente desaparecido. Esto explica lo de los pendientes del sábado que se reasignaron y desaparecieron de ambos lados justo después de restaurar el acceso a Firebase.
  - Se corrigió en dos niveles: la reasignación de pendientes ahora vuelve a guardar si detecta que su cambio se perdió en el camino, y (más importante) la escucha en tiempo real ahora protege cualquier día que se esté guardando en ese momento, sin reemplazarlo hasta que el guardado termine — esto protege contra este mismo problema en cualquier otra función, no solo en reasignar.

- **Aplicado el mismo fix a "reasignar un pendiente" (se me había pasado ese también).** Reasignar de una persona a otra también quita el renglón de la persona original — y tenía la misma vulnerabilidad: la protección contra pérdida de datos podía "recuperar" el pendiente en la persona original, dejándolo viéndose en ambas personas a la vez en vez de solo en la nueva. Ya se registra la exclusión también aquí. Se revisó también el reasignar desde la pantalla de Asignaciones (botón rápido) — ese usa un guardado distinto sin esta vulnerabilidad, no necesitaba corrección.

- **Aplicado el mismo fix del "borro y se regresa" a los pasos de En proceso y Finalizado** (se me había pasado ayer). Mover un pendiente a "En proceso", regresarlo, o finalizarlo también quita un renglón de un lado — y tenían exactamente la misma vulnerabilidad: la protección contra pérdida de datos entre sesiones podía "recuperar" el renglón que se acababa de mover, hacienda que reapareciera en Pendientes después de subirlo a Finalizado. Ya se registra la exclusión en los 3 pasos del flujo (Pendiente→En proceso, En proceso→Pendiente, En proceso→Finalizado), igual que ya se hacía para los borrados directos.

- **Encontrada la causa REAL de "borro y se regresa" — era el propio fix de hace unos días.** La protección para que dos sesiones de la misma persona (celular y computadora) no se borraran el trabajo una a la otra funcionaba comparando "qué tiene el servidor que mi copia local no tiene" para recuperarlo — pero un renglón recién borrado a propósito se ve exactamente igual, para esa comparación, que uno que la copia local simplemente no conocía todavía. Esa misma protección estaba "recuperando" cualquier cosa que se borrara, sin importar quién la borrara ni por qué camino.
  - Se corrigió para que la app recuerde específicamente qué se acaba de borrar a propósito en esta sesión, y esa lista nunca se recupera del servidor al fusionar — sin importar que el servidor todavía la tenga en ese instante. Se probó con una simulación exacta del escenario antes de entregarlo.
  - Aplica a los 3 lugares donde se borra directo: dentro de un reporte, desde la pantalla de Pendientes de Admin, y al autorizar una solicitud de eliminación.

- **Encontrado el bug real y exacto de "borro y se regresa": el borrado nunca se guardaba en la nube.** Cuando Admin borraba un renglón directo, la función quitaba el renglón de la pantalla y guardaba el rastro histórico relacionado (el "descartado" de días viejos) — pero **nunca guardaba el propio día/reporte donde en verdad se había hecho el borrado**. El renglón desaparecía de la vista un instante, pero como ese cambio nunca llegaba a la nube, en cuanto la app volvía a cargar los datos (o se reabría ese mismo reporte), el renglón seguía estando ahí en el servidor y "regresaba" solo. Ya se agregó el guardado que faltaba.

- **Encontrado el motivo exacto por el que pendientes ya resueltos seguían regresando (y no dejaban borrarse).** La revisión de pendientes viejos exigía que el número de guía **y** la fecha de origen coincidieran exacto contra dónde se había resuelto el trabajo. Si ese trabajo se resolvió por un camino distinto al formal (por ejemplo, capturado directo en Despachos en vez de pasar por "En proceso"), nunca quedaba con esa misma fecha de origen — y la comparación tan estricta fallaba en reconocer que sí estaba resuelto, trayendo el pendiente de regreso una y otra vez.
  - Se corrigió para comparar **solo por número de guía**, sin importar la fecha de origen — mucho más confiable.
  - Al borrar un pendiente (desde dentro del reporte o desde la pantalla de Admin), ahora también se revisan **todos los días** por esa misma guía, no solo el de la fecha de origen — para limpiar de una vez cualquier rastro viejo duplicado que pudiera quedar de versiones anteriores.

- **Mejorada la pantalla de "¿Quién eres?"** (Trámite, Ejecutivo, Coordinador) para que se vea organizada aunque haya muchos nombres — cuadrícula de 2 columnas, casillas compactas (formato horizontal: avatar chico + nombre), en orden alfabético, con espacio para ver unas 7 personas por columna sin necesidad de scroll. La casilla de búsqueda arriba (aparece solo si hay más de 6 nombres) sigue ahí para encontrar el tuyo rápido escribiendo las primeras letras cuando el equipo es más grande.

- **Encontrada una causa de fondo más profunda del "se pierden solas" — corregida.** Cada hoja ya se guardaba en su propio espacio (eso evitaba que se pisaran entre tramitadores distintos), pero **cada hoja individual seguía guardándose completa de un jalón**. Si la MISMA persona tenía la app abierta en dos lugares a la vez (celular y computadora, dos pestañas, o una recarga a medias por mala señal), cada sesión guardaba "su propia versión completa" sin saber lo que la otra ya había guardado — la que guardaba al final borraba sin querer lo que la otra había agregado.
  - Ahora, antes de guardar cualquier hoja, la app trae lo que ya existe de verdad en el servidor en ese momento y lo **combina** con la copia local — cualquier previo, despacho, guía revalidada, pendiente o movimiento en proceso que ya esté guardado (por su número de guía) se conserva, aunque la copia local no lo conociera. Se probó con una simulación exacta del escenario (dos sesiones agregando cosas distintas casi al mismo tiempo) antes de entregarlo.
  - Esto aplica tanto al guardado normal como al reintento automático cuando se recupera la señal después de estar offline.

- **Reforzado: reasignar un pendiente ya no debería dejarlo duplicado con la persona original.** Con la sincronización en tiempo real, era posible que una actualización llegara justo a medio camino mientras se guardaban los dos días involucrados (el viejo y el de hoy), dejando el pendiente viéndose en ambos lados por un momento. Se agregó una verificación extra después de guardar para corregirlo solo si eso llegara a pasar.
- **Nuevo: botón para eliminar un pendiente por completo** directo desde la pantalla "Pendientes de todos los tramitadores" (junto al de reasignar) — ya no hace falta entrar al reporte específico para borrarlo.

- **Corregido: "+ Nuevo reporte" no reaccionaba (Querétaro).** Encontré dos causas relacionadas:
  1. La revisión de pendientes viejos sin resolver (se dispara cada vez que un tramitador crea/abre su reporte del día) volvía a recorrer TODO el historial por cada pendiente encontrado — con meses de datos acumulados, esto se vuelve cada vez más lento, al punto de sentirse como que el botón "no hace nada". Se optimizó para que arme un solo índice una vez, en vez de repetir el recorrido completo por cada pendiente — mismo resultado, mucho más rápido sin importar cuánto historial haya.
  2. La aplicación no tenía ningún manejo para errores dentro de funciones que esperan datos (`async`) — si algo fallaba ahí, quedaba en silencio total, sin ningún aviso. Ahora hay un aviso de respaldo para ese tipo de errores, y el botón de "+ Nuevo reporte" en particular ya avisa con un mensaje claro si algo sale mal, en vez de no responder sin explicación.

- **Revisión general preventiva** — no se encontró ningún bug activo nuevo, pero se reforzaron 2 puntos:
  - Si la sincronización en tiempo real llegara a fallar (poco común — problema de conexión persistente), antes la app se quedaría sin enterarse de cambios en silencio. Ahora avisa y cae de vuelta a revisar cada 25 segundos como respaldo, en vez de depender solo de la escucha en tiempo real.
  - El guardado de la copia local (para el modo sin conexión) ahora se limita a como máximo una vez cada 3 segundos — con la sincronización en tiempo real, este guardado se podía disparar muchas veces por minuto si varias personas trabajaban a la vez, lo cual no hacía falta y podía sentirse pesado en celulares más limitados.

- **Corregido: tomar la foto de una guía revalidada a veces "no hacía nada" y regresaba al reporte.** El botón forzaba abrir la cámara directo — en varios celulares, el ir y venir entre la app y la cámara nativa hace que el navegador a veces no entregue bien la foto a la primera. Se quitó ese forzado (ahora el celular puede ofrecer cámara o galería, más confiable en general), y si de todos modos no se detecta ninguna foto, ahora aparece un aviso claro pidiendo intentar de nuevo, en vez de fallar en silencio sin explicación.

- **La sincronización entre personas ahora es en tiempo real, no cada 25 segundos.** Antes, la app "preguntaba" a Firebase cada 25 segundos si había algo nuevo — así que cualquier cambio de otra persona podía tardar hasta ese tiempo en verse. Ahora la app se queda escuchando los cambios directamente: en cuanto alguien guarda, borra, o crea algo en la aduana, todos los demás lo ven casi al instante, sin ningún retraso de por medio. Esto también aplica a las notificaciones push (nuevas asignaciones, asignaciones completadas), que ahora llegan de inmediato en vez de esperar el siguiente ciclo.
  - Se sigue protegiendo a quien esté escribiendo activamente un reporte: los datos se mantienen al día por dentro, pero la pantalla no se le redibuja de golpe mientras está en plena captura (para no interrumpirlo a media escritura) — en cuanto sale de ahí, ya ve todo actualizado sin ningún retraso extra.

- **Corregido bug serio: cosas "Finalizadas" se regresaban solas a Pendientes.** El mecanismo que revisa pendientes viejos sin resolver (se dispara cada vez que alguien vuelve a entrar a su reporte) solo buscaba la copia en el arreglo de Pendientes — pero algo ya "Finalizado" (movido a Previos/Despachos/Guías revalidadas) ya no vive ahí, así que el sistema pensaba "esto nunca se resolvió" y lo volvía a traer, aunque el trabajo ya estuviera bien terminado. Ahora la revisión busca en TODOS los lugares posibles: Pendientes, En proceso, y los 3 controles finales.
- **Corregido: una guía movida a "En proceso" seguía ofreciéndose para "traer" de nuevo**, permitiendo duplicarla por accidente si alguien la volvía a traer sin darse cuenta de que ya la tenía en proceso. Ahora, tanto el botón de "Traer asignaciones pendientes" como el contador de la barra superior revisan Pendientes, En proceso, y los controles finales antes de ofrecer algo de nuevo.

- **Corregido: el botón de "Coordinador" a veces no aparecía la primera vez que entrabas a una aduana.** No era que se borrara nada — era que la pantalla de entrada se dibujaba un instante antes de que terminara de cargar el catálogo de esa aduana, así que por un momento se veía con los datos de la aduana anterior (o vacíos). Al volver a entrar, la carga ya había terminado y se veía bien. Ahora se espera a que el catálogo correcto termine de cargar antes de mostrar la pantalla de entrada (con una pantalla de "Cargando…" breve en medio), así que se ve bien desde la primera vez.

- **Nueva etapa de flujo: Pendientes → En proceso → Finalizado.** Antes, el botón "✅ Listo" de un pendiente lo subía directo a Previos/Despachos/Guías revalidadas. Ahora:
  - Ese botón se renombró a **"🔄 En proceso"** y mueve el pendiente a una nueva sección **"5. Movimientos en proceso"** — todavía no cuenta como terminado.
  - Desde ahí, el botón **"✅ Finalizado"** lo sube de verdad al control que corresponda (Previos, Despachos o Guías revalidadas) — usando exactamente el mismo cálculo de "1er/2do previo" que ya existía, solo que ahora se calcula hasta este momento (más preciso si pasa tiempo entre que se mueve a "en proceso" y se finaliza).
  - También hay un botón **"↩️ Pendiente"** para regresarlo si se movió por error, sin perder ningún dato.
  - El botón de lote (para despachos consolidados) también se actualizó: ahora mueve todo el lote a "En proceso" de un jalón, en vez de finalizarlo directo.
  - Se numeraron de nuevo las secciones: "Otras actividades" pasó de ser la 5 a ser la 6.
  - Los reportes ya guardados (sin este campo) siguen funcionando exactamente igual — no se tocó ni se perdió nada de lo ya construido.

- **Corregida la causa de fondo real: cada hoja ahora se guarda por separado, nunca el día completo de un jalón.** Encontramos el origen verdadero de las "completadas sin rastro": guardar un reporte reescribía el DÍA COMPLETO (todas las hojas de todos los tramitadores de ese día juntas) con lo que esa persona tuviera en su propia copia local — si llevaba un rato editando (su copia se congela a propósito para no interrumpirla) y mientras tanto alguien más guardaba su propia hoja de ese mismo día, al guardar la primera persona borraba por completo lo que la otra acababa de guardar, sin que nadie tocara ningún botón de eliminar. Esto explica por qué pasaba más en aduanas con varios tramitadores trabajando a la vez, como Toluca.
  - Ahora cada hoja vive en su propio espacio dentro del día — guardar o borrar una nunca puede tocar las demás, sin importar qué tan desactualizada esté la copia local de quien guarda.
  - Los reportes ya guardados (formato viejo) se convierten solos al nuevo formato la primera vez que se tocan, sin perder ni duplicar nada — no hace falta ninguna acción manual.
  - Aplica también al reintento automático cuando se recupera la señal después de estar offline.

- **Cerrado por completo el riesgo de "trabajar varios al mismo tiempo" en toda la app.** Los últimos lugares que todavía identificaban una hoja por su posición (guardar foto al instante, solicitar/cancelar eliminación de un renglón, mientras alguien ya está editando activamente su reporte) ahora también usan el ID único de la hoja, igual que el resto. Esto importa especialmente en aduanas con varios tramitadores trabajando a la vez: aunque alguien más guarde su reporte del mismo día justo mientras otra persona sigue editando el suyo, cada acción va a encontrar exactamente la hoja correcta, sin arriesgarse a tocar la de alguien más.

- **Encontrado y corregido: el bug de ayer se me pasó en un lugar clave — la pantalla de "Solicitudes de eliminación".** El botón "✅ Autorizar" que usa Admin/Coordinador ahí también identificaba el renglón por posición, no por un ID único — si la posición cambiaba entre que se abría esa pantalla y que se le daba clic a "Autorizar" (por ejemplo, porque alguien más guardó su reporte de ese mismo día mientras tanto), se podía autorizar/borrar un renglón **distinto** al que realmente se quería, sin que el Admin se diera cuenta. Esto coincide con "nadie con Admin tocó nada" — porque sí se tocó algo, solo que no el renglón correcto. Se corrigió con el mismo criterio de ayer (identificador único + "huella" exacta de cada solicitud, resueltos en el momento del clic).
- También se corrigieron con el mismo criterio: reasignar un pendiente entre tramitadores desde Pendientes, y que un Ejecutivo edite una Observación — ambos tenían el mismo riesgo.

- **Encontrado y corregido un riesgo serio: posible causa real de que renglones "desaparecieran" sin que nadie los borrara.** El guardado y el borrado de una hoja de reporte usaban su **posición** dentro de la lista del día (1º, 2º, 3º...) para saber cuál actualizar — pero esa posición podía cambiar entre que se abría algo para editar/borrar y el momento real del clic (por ejemplo, si mientras tanto otro tramitador guardaba su propio reporte ese mismo día). Guardar o borrar por posición en ese momento podía afectar la hoja equivocada. Se corrigió para que todo se identifique por un ID único de cada hoja (que ya existía desde que se crea), resuelto justo en el momento de la acción — no antes. Esto aplica al guardar cambios de un reporte y al botón de eliminar una hoja completa.

- **Nuevo: resolver de un jalón las "completadas" sin rastro.** En la lista de Completadas, cuando hay varias marcadas "⚠️ sin rastro", aparece un botón "⚠️ Ver solo las N sin rastro" (filtro, para no revisar todas las demás) y un botón **"↩️ Regresar TODAS (N) a pendiente"** que las resuelve todas de un jalón, con una sola confirmación — en vez de tener que darle una por una. No toca nada de los reportes, solo corrige el estatus de cada asignación.

- **Corregido: "2do previo" se marcaba mal cuando otro tramitador ya había hecho esa guía antes.** Al marcar un pendiente "✅ Listo", el sistema contaba cuántas veces esa guía aparecía en CUALQUIER reporte de CUALQUIER tramitador — así que una guía reasignada legítimamente a otra persona (algo normal) le salía marcada "2do previo" a alguien que en realidad la trabajaba por primera vez. Con el sistema de puntos, eso restaba -0.06 injustamente. Ahora el conteo solo considera las veces que ESE MISMO tramitador ya hizo esa guía. De paso, esto también corrige que el aviso de "esta guía ya existe en otro reporte" se saltara en silencio por el mismo motivo.
